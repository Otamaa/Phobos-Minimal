...
public:
	...
	COMPILETIMEEVAL  uint32_t Finalize() {
		if (Index > 0)
		{
			...
			CRC = Memory(&Buffer.Composite, 4, CRC);
			Index = 0;
		}
		return CRC;
	}
...
private:
	...
	COMPILETIMEEVAL uint32_t Update(const void* data, int length) {
		...
		// Process aligned 4-byte blocks
		while (length >= 4)
		{
			uint32_t block;
			std::memcpy(&block, bytes, 4);
			CRC = Memory(&block, 4, CRC);
			bytes += 4;
			length -= 4;
		}
	...
	static inline constexpr uint32_t Table[256] = {
	...
	static_assert(std::size(Table) == 256, "Insufficient CRCTable size!");
};
