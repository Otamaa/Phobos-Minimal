#pragma once

#include <Base/Always.h>
#include <Fixed.h>

typedef enum DirTypes
{
	DIR_N = 0,          // 0
	DIR_NE = 1 << 5,    // 32
	DIR_E = 2 << 5,     // 64
	DIR_SE = 3 << 5,    // 96
	DIR_S = 4 << 5,     // 128
	DIR_SW = 5 << 5,    // 160
	DIR_W = 6 << 5,     // 192
	DIR_NW = 7 << 5,    // 224

	DIR_MIN = 0,
	DIR_MAX = 255,

	DIR_SW_X1 = DirTypes((5 << 5) - 8),  // 152      // Direction of harvester while unloading.
	DIR_SW_X2 = DirTypes((5 << 5) - 16), // 144      // Direction of harvester while unloading.
} DirTypes;
DEFINE_ENUMERATION_OPERATORS(DirTypes);

/**
 *  DirStruct is in stages of 128 (0-32768) (128*256).
 */
#pragma pack(4)
class DirClass : public Fixed
{
public:
	explicit DirClass() : Fixed(0) { }
	explicit DirClass(int raw) { Set_Raw(raw); }
	explicit DirClass(const DirTypes dir) { Set_Dir(dir); }

	void Set_Dir(DirTypes dir) { Fixed::Set_Raw(unsigned(dir % DIR_MAX) * 256); }
	DirTypes Get_Dir() const { return (DirTypes)((((Fixed::Get_Raw() / (32768 / 256)) + 1) / 2) & 255); }

	DirClass& operator /= (const DirTypes nFace) {
		JMP_THIS(0x5B2970);
	}

	// pThis.Value >= (pDir2.Value - pDir3.Value)
	bool CompareToTwoDir(DirClass& pBaseDir, DirClass& pDirFrom)
	{ JMP_THIS(0x5B2990); }

	bool Func_5B29C0(DirClass& pDir2, DirClass& pDir3)
	{ JMP_THIS(0x5B29C0); }
};
#pragma pack()