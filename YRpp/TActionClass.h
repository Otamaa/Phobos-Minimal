...
public:
	...
	struct ActionFuncEntry {
		const char* name;
		DWORD address;
	};

	static std::array<ActionFuncEntry, (size_t)TriggerAction::count> ActionFuncTable;

	bool NOINLINE ExecuteAction(TriggerAction action, HouseClass* pHouse , ObjectClass* pObj , TriggerClass* pTrig , CellStruct* pCell) const
	{
		CONST int idx = (int)action - 1;
		if (action == TriggerAction::None
			|| (TriggerAction)idx == TriggerAction::ChangeZoomLevel
			|| (TriggerAction)idx == TriggerAction::ActivateFirestorm
			|| (TriggerAction)idx == TriggerAction::DeactivateFirestorm
			|| (TriggerAction)idx == TriggerAction::count
			)
			return true;

		return reinterpret_cast<bool(__thiscall*)(const TActionClass*, HouseClass*, ObjectClass*, TriggerClass*, CellStruct*)>
...
#define ACTION_FUNC(name, addr) \
	bool name (HouseClass*, ObjectClass* , TriggerClass*, CellStruct*) { JMP_THIS(addr); }

		ACTION_FUNC(AllowWin, 0x6E1DC0);
		...
		ACTION_FUNC(AnnounceWin, 0x6E0440);
		ACTION_FUNC(AnnounceLose, 0x6E0460);
