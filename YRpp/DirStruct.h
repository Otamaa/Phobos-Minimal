...
public:
	...
	COMPILETIMEEVAL DirStruct(const DirStruct& nDir) noexcept :
		Raw { nDir.Raw }
	{  }

	COMPILETIMEEVAL explicit DirStruct() noexcept :
		Raw { 0 }
	{ }

	COMPILETIMEEVAL explicit DirStruct(int raw) noexcept :
	...
	COMPILETIMEEVAL FORCEDINLINE DirStruct& operator /= (const short nFace) {
		 this->Raw /= nFace;
		return *this;
	}

	COMPILETIMEEVAL FORCEDINLINE DirStruct& operator += (const DirStruct& rhs) {
		this->Raw += rhs.Raw;
		return *this;
	}
	...
	COMPILETIMEEVAL FORCEDINLINE DirStruct operator - (const DirStruct& rhs) const {
		return DirStruct(*this) -= rhs;
	}

	//COMPILETIMEEVAL FORCEDINLINE DirStruct operator - () const {
	...
	COMPILETIMEEVAL FORCEDINLINE void SetDir(DirType dir) { Raw = ((unsigned short)((unsigned char)dir * 256)); }
	COMPILETIMEEVAL FORCEDINLINE DirType GetDirFixed() const { return (DirType)((((this->Raw / (32768 / 256)) + 1) / 2) & (int)DirType::Max); }
	COMPILETIMEEVAL FORCEDINLINE DirType GetDir() const { return (DirType)(this->Raw / 256); }
	...
	COMPILETIMEEVAL FORCEDINLINE  size_t Getvalue8() const {
		return this->GetValue<3>();
	}

	COMPILETIMEEVAL FORCEDINLINE  size_t Getvalue32() const {
		return this->GetValue<5>();
	}

	COMPILETIMEEVAL FORCEDINLINE  size_t Getvalue256() const {
