...
public:
	...
	static constexpr MouseCursor& GetDefaultCursor(MouseCursorType cursor)
		{ return DefaultCursors[static_cast<int>(cursor)]; }

	static constexpr MouseCursor& GetCursor(MouseCursorType cursor)
		{ return DefaultCursors[static_cast<int>(cursor)]; }

	static constexpr MouseCursor& GetCursor(size_t cursor)
		{ return DefaultCursors[cursor]; }

	constexpr FORCEINLINE int GetFrameRate() const {
		return FrameRate;
	}

	constexpr FORCEINLINE int GetMouseFrame(bool wsmall) const
	{
		return !wsmall || SmallFrame == -1 ?
			StartFrame : SmallFrame;
	}

	constexpr FORCEINLINE int GetMouseFrameCount(bool wsmall) const
	{
	...
	constexpr FORCEINLINE Point2D GetMouseHotSpot(const SHPStruct* pShape) const
	{
		Point2D nResult { 0 , 0 };
...
{
	constexpr MouseCursorDataStruct() = default;
	...
	constexpr MouseCursorDataStruct(MouseCursor const& nData , int miniInterval) :
		OriginalData { nData },
		SmallFrameRate { miniInterval }
	{ }

	constexpr ~MouseCursorDataStruct() noexcept = default;
...
public:
	...
	bool ThumbActive; //unknown_byte_5544
