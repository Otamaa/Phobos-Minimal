...
public:
	...
	{
		Datas = (MapSurfaceData*)Allocate(sizeof(MapSurfaceData) * nCount);
		States = (bool*)Allocate(sizeof(bool) * nCount);
...
public:
	...
	//Destructor
	virtual ~VeinholeMonsterClass() override;
  ...
  ObjectClass* Attacker, bool IgnoreDefenses, bool PreventPassengerEscape, HouseClass* pAttackingHouse) override;//JMP_THIS(0x74D5D0);
	...
	VeinholeMonsterClass(CellStruct* pWhere);

public:

	VeinholeLogic GrowthLogic;
	int CurrentState;
	int NextState;
	int  MonsterFrameIdx;
...
inline void __cdecl UpdateAllVeinholes()
;//{ JMP(0x74CDF0); }
