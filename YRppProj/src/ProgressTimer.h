...
public:
	...
	constexpr ProgressTimer(const ProgressTimer&) = default;
	constexpr ProgressTimer& operator = (const ProgressTimer&) = default;
	constexpr ProgressTimer(int duration) { this->Start(duration); }

	constexpr void Start(int duration) {
		this->Timer.Start(duration);
	}

	constexpr void Start(int duration, int step) {
		this->Step = step;
		this->Start(duration);
	}
