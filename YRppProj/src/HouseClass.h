...
#include <Helpers/IndexBitfield.h>
...
struct AngerStruct
{
	...
	constexpr bool operator == (const AngerStruct& tAnger) const
	{
		return (House == tAnger.House &&
				AngerLevel == tAnger.AngerLevel);
	}

	constexpr bool operator != (const AngerStruct& that) const
	{
...
struct ScoutStruct
{
	...
	//need to define a == operator so it can be used in array classes
	constexpr bool operator == (const ScoutStruct& tScout) const
	{
		return (House == tScout.House &&
				IsPreferred == tScout.IsPreferred);
	}

	constexpr bool operator != (const ScoutStruct& that) const
	{
...
#pragma endregion
	...
	constexpr bool DoInfantrySelfHeal() const
		{ return this->InfantrySelfHeal > 0; }

	int GetInfSelfHealStep() const
		;// { JMP_THIS(0x50D9E0); }

	constexpr bool DoUnitsSelfHeal() const
	...
	constexpr bool HasFullPower() const {
		return !this->PowerDrain || this->PowerOutput >= this->PowerDrain;
	}

	constexpr bool HasLowPower() const {
		return this->PowerDrain && this->PowerOutput < this->PowerDrain;
	}
	...
	constexpr static HouseClass* FindBySideIndex(int index) {
...
public:
	...
	CDTimerClass			RepairTimer; // for AI
	...
	UnitTrackerClass	  CollectedCrates;	//YES, THIS IS HOW WW WASTES TONS OF RAM
	int                   NumAirpads;
	int                   NumBarracks;
	int                   NumWarFactories;
	int                   NumConYards;
...
//	IndexBitfield<HouseTypeClass *> RadarVisibleTo; // these house types(!?!, fuck you WW) can see my radar
	...
	CounterClass<GameAllocator<int>> OwnedBuildingTypes;
	CounterClass<GameAllocator<int>> OwnedUnitTypes;
	CounterClass<GameAllocator<int>> OwnedInfantryTypes;
	CounterClass<GameAllocator<int>> OwnedAircraftTypes;
	...
	CounterClass<GameAllocator<int>> ActiveBuildingTypes;
	CounterClass<GameAllocator<int>> ActiveUnitTypes;
	CounterClass<GameAllocator<int>> ActiveInfantryTypes;
	CounterClass<GameAllocator<int>> ActiveAircraftTypes;

		// Used for: Counting objects produced from Factory
		// not altered when things get taken over or removed
		// BuildLimit < 0 validation uses this
	CounterClass<GameAllocator<int>> FactoryProducedBuildingTypes;
	CounterClass<GameAllocator<int>> FactoryProducedUnitTypes;
	CounterClass<GameAllocator<int>> FactoryProducedInfantryTypes;
	CounterClass<GameAllocator<int>> FactoryProducedAircraftTypes;
	...
	CDTimerClass TeamDelayTimer; // for AI attacks
