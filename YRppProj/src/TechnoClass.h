...
public:
	...
	FlashData Flashing;
	StageClass Animation; // how the unit animates
	...
	CDTimerClass IdleActionTimer; // MOO
	CDTimerClass RadarFlashTimer;
	CDTimerClass TargetingTimer; //Duration = 45 on init!
	CDTimerClass  IronCurtainTimer;
	CDTimerClass IronTintTimer; // how often to alternate the effect color
	int              IronTintStage; // ^
	CDTimerClass AirstrikeTimer;
	CDTimerClass AirstrikeTintTimer; // tracks alternation of the effect color
	...
	CDTimerClass InfantryBlinkTimer; // Rules->InfantryBlinkDisguiseTime , detects mirage firing per description
	CDTimerClass DisguiseBlinkTimer; // disguise disruption timer
	...
	*/
	...
	StageClass CloakProgress; // phase from [opaque] -> [fading] -> [transparent] , [General]CloakingStages= long
	CDTimerClass CloakDelayTimer; // delay before cloaking again
...
//	PROTECTED_PROPERTY(BYTE, align2A_5_6_7[3]);
 ...
 // old Brute hack
	...
	DoorClass UnloadTimer; // times the deploy, unload, etc. cycles ,DoorClass

	FacingClass BarrelFacing;
	FacingClass PrimaryFacing; //Facing
	FacingClass SecondaryFacing; // TurretFacing
