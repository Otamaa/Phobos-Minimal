#pragma once

struct ScriptActionNode
{
public:
	unsigned int BuildINIEntry(char* pTr)
		;// { JMP_THIS(0x723CE0); }

	ScriptActionNode(const char* pTr);

	constexpr ScriptActionNode(int Act, int Arg) noexcept :
		Action { Act }, Argument { Arg }
	{ }

	constexpr ScriptActionNode(TeamMissionType Act, int Arg) noexcept :
		Action { Act }, Argument { Arg }
	{ }

	constexpr ScriptActionNode() noexcept = default;
	constexpr ~ScriptActionNode() noexcept = default;

	constexpr bool operator==(ScriptActionNode const& rhs) const
	{
		//return Action == rhs.Action && Argument == rhs.Argument;
		return false; // so , umm we dont really care actually ,.. but , eh whatever
	}

	constexpr bool operator!=(ScriptActionNode const& rhs) const
	{
		//return !((*this) == rhs);
		return true; // so , umm we dont really care actually ,.. but , eh whatever
	}

public:
	TeamMissionType Action { TeamMissionType::none };
	int Argument { 0 };
};