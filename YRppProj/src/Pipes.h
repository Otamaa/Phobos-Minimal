...
public:
	...
	explicit LCWPipe(BOOL bControl, int nBlockSize) : Pipe {}
	{
		...
		this->Buffer = Allocate(this->BlockSize + this->SafetyMargin);
		this->Buffer2 = Allocate(this->BlockSize + this->SafetyMargin);
	...
	virtual ~LCWPipe() override final
	{
		...
		Deallocate(this->Buffer2);
	}

	virtual int Flush() override final;//{ JMP_THIS(0x5522D0); }
