...
public:
	...
	//Constructors
	GadgetClass(int nX, int nY, int nWidth, int nHeight, GadgetFlag eFlag, bool bSticky);
	GadgetClass(RectangleStruct rect, GadgetFlag eFlag, bool bSticky);
	GadgetClass(GadgetClass& another);
...
class NOVTABLE TextLabelClass : public GadgetClass
{
	...
	TextLabelClass(wchar_t* txt, int x, int y, int ColorSchemeIndex, TextPrintType style);
	TextLabelClass(wchar_t* txt, Point2D point, int ColorSchemeIndex, TextPrintType style);
