...
public:
	...
	constexpr BuildType(int itemIndex, AbstractType itemType) noexcept :
		ItemIndex { itemIndex },
		ItemType { itemType },
		Cat { BuildCat::DontCare },
		CurrentFactory { nullptr },
	...
	constexpr BuildType&operator =(const BuildType&) = default;
	constexpr BuildType(const BuildType&) = default;
	constexpr BuildType&operator=(BuildType&&) = default;

	constexpr bool operator == (const BuildType& rhs) const
	{
