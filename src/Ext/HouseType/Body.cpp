...
void HouseTypeExtData::InheritSettings(HouseTypeClass* pThis)
{
	if (auto ParentCountry = pThis->FindParentCountry()) {
		if (const auto ParentData = HouseTypeExtContainer::Instance.Find(ParentCountry)) {
			...
			this->Disguise = ParentData->Disguise;
...
HRESULT __stdcall FakeHouseTypeClass::__Load(IStream* pStm)
{
	HRESULT hr = this->HouseTypeClass::Load(pStm);

	if (SUCCEEDED(hr)) {
		if (!HouseTypeExtContainer::Instance.LoadByKey(this, pStm))
			return PHOBOS_E_EXTDATA_LOAD_FAILED;
	}

	return hr;
}
DEFINE_FUNCTION_JUMP(VTABLE, 0x7EAB6C, FakeHouseTypeClass::__Load)

HRESULT __stdcall FakeHouseTypeClass::__Save(IStream* pStm, BOOL fClearDirty)
{
	HRESULT hr = this->HouseTypeClass::Save(pStm, fClearDirty);

	if (SUCCEEDED(hr)) {
		if (!HouseTypeExtContainer::Instance.SaveByKey(this, pStm))
			return PHOBOS_E_EXTDATA_SAVE_FAILED;
	}

	return hr;
}
DEFINE_FUNCTION_JUMP(VTABLE, 0x7EAB70, FakeHouseTypeClass::__Save)