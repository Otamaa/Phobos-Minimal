	...
	__forceinline void Blit_Impl(void* dst, byte* src, int len, int zval, WORD* zbuf, WORD* abuf, int alvl, int warp)
	{
		...
		if constexpr (Level == Simd::Level::AVX512 && CompileAvx512)
		{
			...
			while (len >= ChunkSize)
			{
				...
				if (zMask)
				{
					...
					const __mmask16 gatherMask = zMask & ~maxIndexMask;
					...
					const __mmask16 fillMask = zMask & maxIndexMask;
					if (fillMask)
					{
					...
					_mm256_mask_storeu_epi16(zbuf, zMask, zWriteVec);
				}
		...
		if constexpr (Level == Simd::Level::SSE2)
		{
			...
			while (len >= ChunkSize)
			{
				...
				if (zval < 0)
				{
					activeMask16 = _mm_set1_epi16(static_cast<short>(-1));
				}
				else if (zval <= 0xFFFF)
				{
					const __m128i zbuf16 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(zbuf));
					activeMask16 = Sse2_CmpGtEpu16(zbuf16, zMask16);
				}
