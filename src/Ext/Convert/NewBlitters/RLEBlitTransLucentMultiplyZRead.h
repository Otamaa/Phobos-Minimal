...
// ============================================================================
//  RLEBlitTransLucentMultiplyZRead<T>
//
//  Backported from 0x1004CB00. Same per-channel modulate as the plain sibling
//  - see BlitTransLucentMultiplyZRead.h for the division note.
//
//  The pseudocode appears to fetch the palette entry twice, once for `v25` and
//  again inside the expression for `v32`. It does not: 0x1004CC64 reads it
//  once into ECX and everything downstream uses that. IDA simply failed to
//  common the subexpression.
//
...
template<typename T>
class RLEBlitTransLucentMultiplyZRead final : public RLEBlitter
{
public:
	inline explicit RLEBlitTransLucentMultiplyZRead(T* data, int shadecount) noexcept
	{
		PaletteData = data;
		AlphaRemapper = AlphaLightingRemapClass::Global->FindOrAllocate(shadecount);
	}

	virtual ~RLEBlitTransLucentMultiplyZRead() override final = default;
