...
// ============================================================================
//  RLEBlitTransLucentDoubleMultiplyZRead<T>
//
//  Backported from 0x1004C840. Same combine as the plain sibling - see
//  BlitTransLucentDoubleMultiplyZRead.h and BlitterMath.h.
//
//  As in the RLE Multiply and Luna, the pseudocode appears to fetch the
//  palette entry twice; the asm reads it once at 0x1004C9A3.
//
...
template<typename T>
class RLEBlitTransLucentDoubleMultiplyZRead final : public RLEBlitter
{
public:
	inline explicit RLEBlitTransLucentDoubleMultiplyZRead(T* data, int shadecount) noexcept
	{
		PaletteData = data;
		AlphaRemapper = AlphaLightingRemapClass::Global->FindOrAllocate(shadecount);
	}

	virtual ~RLEBlitTransLucentDoubleMultiplyZRead() override final = default;
