...
ASMJIT_PATCH(0x7128C0, TechnoTypeClass_LoadFromINI_Weapons1, 6)
{
	GET(TechnoTypeClass*, pThis, EBP);
	GET(CCINIClass*, pINI, ESI);
	TechnoTypeExtData::ReadWeaponStructDatas(pThis, pINI);
	return 0x712A8F;
}
...
SHPCaches* __fastcall FakeTechnoTypeClass::__GetCameo(TechnoTypeClass* pThis) {
	return TechnoTypeExtData::CameoIsElite(pThis, HouseClass::CurrentPlayer) ?
...
ASMJIT_PATCH(0x747BCF, UnitTypeClass_LoadFromINI_Turrets, 5)
{
	...
	if (pThis->Gunner)
		TechnoTypeExtData::LoadTurrets(pThis, pINI);
...
ASMJIT_PATCH(0x70DC70, TechnoClass_SwitchGunner, 6)
{
	...
	if (!pType->IsChargeTurret)
	{
		...
		pThis->CurrentTurretNumber = *TechnoTypeExtData::GetTurretWeaponIndex(pType, nWeaponIdx);
...
ASMJIT_PATCH(0x717890, TechnoTypeClass_SetWeaponTurretIndex, 8)
{
	...
	*TechnoTypeExtData::GetTurretWeaponIndex(pThis, nWeaponIdx) = nTurIdx;
	return 0x71789F;
}