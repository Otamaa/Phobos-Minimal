	...
	if (pExt->HasSplitBehavior())
	{
		...
		if (WeaponTypeClass* pWeapon = GetWeapon())
		{
			...
			if (!pExt->Splits.Get())
			{
				...
				{
					//if (EnumFunctions::IsCellEligible(pCell, pExt->Airburst_AffectsCell))
			...
			for (int i = 0; i < cluster; ++i)
			{
				...
				if (pTarget)
				{
					...
					if (const auto pBullet = pSplitExt->CreateBullet(pTarget, pThis->Owner, pWeapon))
					{
						...
						pBullet->MoveTo(pThis->Location, { cos_rad * flatSpeed,sin_rad * flatSpeed, static_cast<double>(-pBullet->Speed) });
