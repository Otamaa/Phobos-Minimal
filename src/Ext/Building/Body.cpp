...
bool BuildingExtContainer::Save(BuildingClass* key, IStream* pStm)
{
	if constexpr (CanThisSaveToStream<BuildingExtData>)
	{
		...
		// get the value data
		BuildingExtData* const buffer = this->Find(key);
...
bool BuildingExtContainer::Load(BuildingClass* key, IStream* pStm)
{
	if constexpr (CanThisLoadFromStream<BuildingExtData>)
	{
		// this really shouldn't happen
		if (!key)
		{
		...
		if (reader.Expect(BuildingExtData::Canary) && reader.RegisterChange(buffer))
		{
