...
{
	GET(BuildingClass*, pThis, ECX);
	TechnoExtData::UpdateFactoryQueues(pThis);
	return 0;
}
...
{
	GET(BuildingClass*, pThis, ESI);
	R->EAX(TechnoExtData::IsBaseNormal(pThis));
	return 0x440C2C;
}
...
ASMJIT_PATCH(0x44A04C, BuildingClass_Destruction_CopyEMPDuration, 6)
{
	...
	WarheadTypeExtData::UpdateSparkleAnim(pUnit, pBuilding);
...
ASMJIT_PATCH(0x519FAF, InfantryClass_UpdatePosition_EngineerRepairsFriendly, 6)
{
	...
	{
		auto pRubble = TechnoExtData::CreateBuilding(Target,
...
ASMJIT_PATCH(0x7004AD, TechnoClass_GetActionOnObject_Saboteur, 0x6)
{
	...
	{
		infiltratable = TechnoExtData::GetiInfiltrateActionResult(pThis, pBldObject) != Action::None;
	}
...
ASMJIT_PATCH(0x51EE6B, InfantryClass_GetActionOnObject_Saboteur, 6)
{
	...
	if (auto pBldObject = cast_to<BuildingClass*>(pObject))
	{
		if (pThis->Owner && !pThis->Owner->IsAlliedWith(pBldObject))
		{
			...
			switch (TechnoExtData::GetiInfiltrateActionResult(pThis, pBldObject))
			{
			case Action::Move:
...
ASMJIT_PATCH(0x51B2CB, InfantryClass_SetTarget_Saboteur, 0x6)
{
	...
	{
		const auto nResult = TechnoExtData::GetiInfiltrateActionResult(pThis, pBldObject);
...
ASMJIT_PATCH(0x51A521, InfantryClass_UpdatePosition_ApplyC4, 0xA)
{
	...
	if (!TechnoExtData::ApplyC4ToBuilding(pThis, pBuilding, false))
		return RetFail;
...
ASMJIT_PATCH(0x519FF8, InfantryClass_UpdatePosition_Saboteur, 6)
{
	...
	const auto nResult = TechnoExtData::GetiInfiltrateActionResult(pThis, pBuilding);
	...
	if (nResult == Action::Move) // this one will Infiltrate instead
	{
		...
		{
			if (!TechnoExtData::ApplyC4ToBuilding(pThis, pBuilding, true))
				return SkipInfiltrate;
...
ASMJIT_PATCH(0x44F7A0, BuildingClass_UpdateDisplayTo, 6)
{
	...
	TechnoExtData::UpdateDisplayTo(B);
	return 0x44F813;
}
...
ASMJIT_PATCH(0x44D8A1, BuildingClass_UnloadPassengers_Unload, 6)
{
	...
	TechnoExtData::KickOutHospitalArmory(B);
	return 0;
}
...
{
	GET(BuildingClass*, pBld, ESI);
	TechnoExtData::EvalRaidStatus(pBld);
	return 0;
...
{
	GET(BuildingClass*, pBld, ESI);
	TechnoExtData::UpdateSensorArray(pBld);
	return 0;
