...
#pragma endregion
		...
		this->ExtraThreat_Enabled = ExtraThreat_IsThreat.Get(FakeRulesClass::Instance()->ExtraThreat_IsThreat) != 0.0
			|| ExtraThreat_InRange.Get(FakeRulesClass::Instance()->ExtraThreat_InRange) != 0.0
			|| ExtraThreatCoefficient_InRangeDistance.Get(FakeRulesClass::Instance()->ExtraThreatCoefficient_InRangeDistance) != 0.0
			|| ExtraThreatCoefficient_Facing.Get(FakeRulesClass::Instance()->ExtraThreatCoefficient_Facing) != 0.0
			|| ExtraThreatCoefficient_DistanceToLastTarget.Get(FakeRulesClass::Instance()->ExtraThreatCoefficient_DistanceToLastTarget) != 0.0;
	...
	if (pArtIni && pArtIni->GetSection(pArtSection))
	{
		...
		for (size_t i = 0; ; ++i) {
			auto nHitBuff = Nullable<CoordStruct>()(exArtINI, pArtSection, (std::string("HitCoordOffset") + std::to_string(i)).c_str(), false);
...

		{
			if (this->ExtraBarrelCount > 0) {
				std::string basse("ExtraBarrelOffset");

				for (int i = 0; i < this->ExtraBarrelCount; ++i) {
					auto extraBarrelOffset = Valueable<int>()(exArtINI, pArtSection, (basse + std::to_string(i)).c_str(), false);
...

		{
			if (this->ExtraTurretCount > 0) {
				std::string base("ExtraTurretOffset");

				for (int i = 0; i < this->ExtraTurretCount; ++i) {
					auto extraTurretOffset = Valueable<CoordStruct>()(exArtINI, pArtSection, (base + std::to_string(i)).c_str() , false);
...
void TechnoTypeExtData::Serialize(T& Stm) {
	Stm
		...
		// SKIPPED: .Process(this->BarrelImageData) - VoxelStruct holds VXL/HVA resources resolved at
		// type-init from the mix files; there is no Process<VoxelStruct> overload and the
		// pointers are not valid across a save/load. Re-resolved on load instead.
		// SKIPPED: .Process(this->TurretImageData) - VoxelStruct holds VXL/HVA resources resolved at
		// type-init from the mix files; there is no Process<VoxelStruct> overload and the
		// pointers are not valid across a save/load. Re-resolved on load instead.
		// SKIPPED: .Process(this->SpawnAltData) - VoxelStruct holds VXL/HVA resources resolved at
		// type-init from the mix files; there is no Process<VoxelStruct> overload and the
		// pointers are not valid across a save/load. Re-resolved on load instead.
