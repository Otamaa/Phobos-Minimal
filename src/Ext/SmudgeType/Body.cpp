...
HRESULT __stdcall FakeSmudgeTypeClass::__Load(IStream* pStm)
{
	HRESULT hr = this->SmudgeTypeClass::Load(pStm);

	if (SUCCEEDED(hr)) {
		if (!SmudgeTypeExtContainer::Instance.LoadByKey(this, pStm))
			return PHOBOS_E_EXTDATA_LOAD_FAILED;
	}

	return hr;
}
DEFINE_FUNCTION_JUMP(VTABLE, 0x7F353C, FakeSmudgeTypeClass::__Load)

HRESULT __stdcall FakeSmudgeTypeClass::__Save(IStream* pStm, BOOL fClearDirty)
{
	HRESULT hr = this->SmudgeTypeClass::Save(pStm, fClearDirty);

	if (SUCCEEDED(hr)) {
		if (!SmudgeTypeExtContainer::Instance.SaveByKey(this, pStm))
			return PHOBOS_E_EXTDATA_SAVE_FAILED;
	}

	return hr;
}
DEFINE_FUNCTION_JUMP(VTABLE, 0x7F3540, FakeSmudgeTypeClass::__Save)