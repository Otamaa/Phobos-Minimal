...
ASMJIT_PATCH(0x46920B, BulletClass_Detonate, 6)
{
	...
	if (snapped && pWeaponExt && WeaponTypeExtData::conductAbduction(pThis->WeaponType, pThis->Owner, pThis->Target, coords)) {
	...
	if (targetStillOnMap) {
		...
		if (snapped && WeaponTypeExtData::applyOccupantDamage(pThis)) {
			// ..and neuter the bullet, since it's not supposed to hurt the prisoner after the abduction
			pThis->Health = 0;
			pThis->DamageMultiplier = 0;
			pThis->Limbo();
