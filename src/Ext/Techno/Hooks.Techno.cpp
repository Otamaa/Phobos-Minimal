...
ASMJIT_PATCH(0x7014D5, TechnoClass_SetOwningHouse_Additional, 6)
{
	...
	if (auto pBuilding = cast_to<BuildingClass*, false>(pThis)) {
		...
		if (!nTunnelVec || HouseExtData::FindSameTunnel(pBuilding))
			return 0x0;

		for (auto nPos = nTunnelVec->Vector.begin();
			nPos != nTunnelVec->Vector.end(); ++nPos) {
			HouseExtData::KillFootClass(*nPos, nullptr);
		}
...
ASMJIT_PATCH(0x702E64, TechnoClass_RegisterDestruction_Bounty, 6)
{
	...
	TechnoExtData::GiveBounty(pVictim, pKiller);
	...
	if(pType)
	{
		...
		for (auto i = 0; i < WeaponCount; ++i) {
			...
			if (auto const pWeapon = pType->GetWeapon(i)->WeaponType) {
				TechnoExtData::InitWeapon(pThis, pType, pWeapon, i, pCapturer, pParasite, pTemporal, "Weapon", IsFoot);
			}

			if (auto const pWeaponE = pType->GetEliteWeapon(i)->WeaponType) {
				TechnoExtData::InitWeapon(pThis, pType, pWeaponE, i, pCapturer, pParasite, pTemporal, "EliteWeapon", IsFoot);
			}
		}
		...
		if (!pExt->AE.flags.HasTint && pExt->CurrentShieldType)
			pExt->Tints.Update();
