...
#include <EASTL/string.h> /*for size_t*/
...
namespace lodepng {
#ifdef LODEPNG_COMPILE_DECODER
/*Same as lodepng_decode_memory, but decodes to an eastl::vector. The colortype
is the format to output the pixels to. Default is RGBA 8-bit per channel.*/
unsigned decode(eastl::vector<unsigned char>& out, unsigned& w, unsigned& h,
                const unsigned char* in, size_t insize,
                LodePNGColorType colortype = LCT_RGBA, unsigned bitdepth = 8);
unsigned decode(eastl::vector<unsigned char>& out, unsigned& w, unsigned& h,
                const eastl::vector<unsigned char>& in,
                LodePNGColorType colortype = LCT_RGBA, unsigned bitdepth = 8);
#ifdef LODEPNG_COMPILE_DISK
/*
...
#ifdef LODEPNG_COMPILE_ENCODER
/*Same as lodepng_encode_memory, but encodes to an eastl::vector. colortype
is that of the raw input data. The output PNG color type will be auto chosen.*/
unsigned encode(eastl::vector<unsigned char>& out,
                const unsigned char* in, unsigned w, unsigned h,
                LodePNGColorType colortype = LCT_RGBA, unsigned bitdepth = 8);
unsigned encode(eastl::vector<unsigned char>& out,
                const eastl::vector<unsigned char>& in, unsigned w, unsigned h,
                LodePNGColorType colortype = LCT_RGBA, unsigned bitdepth = 8);
#ifdef LODEPNG_COMPILE_DISK
/*
...
to handle such files and decode in-memory.
*/
unsigned encode(const eastl::string& filename,
                const unsigned char* in, unsigned w, unsigned h,
                LodePNGColorType colortype = LCT_RGBA, unsigned bitdepth = 8);
unsigned encode(const eastl::string& filename,
                const eastl::vector<unsigned char>& in, unsigned w, unsigned h,
...
#ifdef LODEPNG_COMPILE_CPP
/* The LodePNG C++ wrapper uses eastl::vectors instead of manually allocated memory buffers. */
namespace lodepng {
#ifdef LODEPNG_COMPILE_PNG
...
#ifdef LODEPNG_COMPILE_DISK
/*
Load a file from disk into an eastl::vector.
return value: error code (0 means ok)

NOTE: Wide-character filenames are not supported, you can use an external method
to handle such files and decode in-memory
*/
unsigned load_file(eastl::vector<unsigned char>& buffer, const eastl::string& filename);

/*
Save the binary data in an eastl::vector to a file on disk. The file is overwritten
without warning.

NOTE: Wide-character filenames are not supported, you can use an external method
to handle such files and encode in-memory
*/
unsigned save_file(const eastl::vector<unsigned char>& buffer, const eastl::string& filename);
#endif /* LODEPNG_COMPILE_DISK */
#endif /* LODEPNG_COMPILE_PNG */
...
unsigned decompress(eastl::vector<unsigned char>& out, const unsigned char* in, size_t insize,
                    const LodePNGDecompressSettings& settings = lodepng_default_decompress_settings);

/* Zlib-decompress an eastl::vector */
unsigned decompress(eastl::vector<unsigned char>& out, const eastl::vector<unsigned char>& in,
                    const LodePNGDecompressSettings& settings = lodepng_default_decompress_settings);
#endif /* LODEPNG_COMPILE_DECODER */

#ifdef LODEPNG_COMPILE_ENCODER
/* Zlib-compress an unsigned char buffer */
unsigned compress(eastl::vector<unsigned char>& out, const unsigned char* in, size_t insize,
                  const LodePNGCompressSettings& settings = lodepng_default_compress_settings);

/* Zlib-compress an eastl::vector */
unsigned compress(eastl::vector<unsigned char>& out, const eastl::vector<unsigned char>& in,
                  const LodePNGCompressSettings& settings = lodepng_default_compress_settings);
#endif /* LODEPNG_COMPILE_ENCODER */
#endif /* LODEPNG_COMPILE_ZLIB */
} /* namespace lodepng */
...
The C++ version has extra functions with eastl::vectors in the interface and the
...
eastl::vectors and eastl::strings in C++ can be incompatible.
...
int main(int argc, char *argv[]) {
  ...
  //load and decode
  eastl::vector<unsigned char> image;
  unsigned width, height;
  unsigned error = lodepng::decode(image, width, height, filename);

  //if there's an error, display it
  if(error) eastl::cout << "decoder error " << error << ": " << lodepng_error_text(error) << eastl::endl;
...
*) 04 jun 2007: improved support for Visual Studio 2005: crash with accessing
    invalid eastl::vector element [0] fixed, and level 3 and 4 warnings removed
...
*) 07 sep 2005: (!) Changed to eastl::vector interface
