...
public:
	...
	bool Delete(int index, bool allow_shrink = true)
	{
		...
		{
			std::memmove(&(m_vector[index]), &(m_vector[index + 1]), (m_activeCount - index - 1) * sizeof(T));
		}
		m_activeCount--;
		if (allow_shrink)
		{
	...
	{
		if (m_vectorMax >= capacity)
		{
			return true;
		}

		return SimpleVecClass<T>::Resize(capacity);
	}

	bool Add_Unique(const T& item) {
		return this->Find_Index(item) == -1 && this->Add(item);
	}

	void Set_Count(int newCount) {
		m_activeCount = newCount;
	}
