...
void JxlSimpleLosslessPrepareHeader(JxlSimpleLosslessFrameState *frame,
  ...
  if (add_image_header) {
    ...
    if (frame->color_encoding.is_float && frame->bitdepth == 16) {
    ...
    if (!(frame->color_encoding.primaries == JXL_PRIMARIES_SRGB
          && frame->color_encoding.transfer_function == JXL_TRANSFER_FUNCTION_SRGB)) {
      output->Write(1, 0);    // color_encoding.all_default false
      output->Write(1, 0);    // color_encoding.want_icc false
      if (frame->color_encoding.color_space == JXL_COLOR_SPACE_GRAY
          && (frame->nb_chans == 1 || frame->nb_chans == 2)) {
        output->Write(2, 1);    // grayscale
      } else {
        output->Write(2, 0);    // default to RGB
      }
      if (frame->color_encoding.white_point == JXL_WHITE_POINT_DCI) {
        output->Write(2, 11);  // DCI
      } else if (frame->color_encoding.white_point == JXL_WHITE_POINT_E) {
        output->Write(2, 10);  // E
      } else {
        output->Write(2, 1);  // default to D65
      }
      if (frame->color_encoding.primaries == JXL_PRIMARIES_2100) {
        output->Write(2, 0b10); // primaries: 2 + u(4)
        output->Write(4, 7);    // 2100
      } else if (frame->color_encoding.primaries == JXL_PRIMARIES_P3) {
        output->Write(2, 0b10); // primaries: 2 + u(4)
        output->Write(4, 9);    // P3
      } else {
        output->Write(2, 1);    // default to sRGB
      }
      if (frame->color_encoding.transfer_function == JXL_TRANSFER_FUNCTION_PQ) {
        output->Write(1, 0);    // no gamma transfer function
        output->Write(2, 0b10); // tf: 2 + u(4)
        output->Write(4, 14);   // tf of PQ
      } else if (frame->color_encoding.transfer_function == JXL_TRANSFER_FUNCTION_DCI) {
        output->Write(1, 0);    // no gamma transfer function
        output->Write(2, 0b10); // tf: 2 + u(4)
        output->Write(4, 15);   // tf of DCI
      } else if (frame->color_encoding.transfer_function == JXL_TRANSFER_FUNCTION_HLG) {
        output->Write(1, 0);    // no gamma transfer function
        output->Write(2, 0b11); // tf: 18 + u(6)
        output->Write(6, 0);    // tf of HLG
      } else if (frame->color_encoding.transfer_function == JXL_TRANSFER_FUNCTION_LINEAR) {
        output->Write(1, 0);    // no gamma transfer function
        output->Write(2, 0b10); // tf: 2 + u(4)
        ...
        output->Write(1, 0);    // no gamma transfer function
        output->Write(2, 0b10); // tf: 2 + u(4)
        output->Write(4, 11);   // tf default to sRGB
      }
      if (frame->color_encoding.rendering_intent == JXL_RENDERING_INTENT_PERCEPTUAL) {
        output->Write(2, 0);    // perceptual rendering intent
      } else if (frame->color_encoding.rendering_intent == JXL_RENDERING_INTENT_SATURATION) {
        output->Write(2, 0b10); // rendering intent: 2 + u(4)
        output->Write(4, 0);    // saturation rendering intent
      } else if (frame->color_encoding.rendering_intent == JXL_RENDERING_INTENT_ABSOLUTE) {
        output->Write(2, 0b10); // rendering intent: 2 + u(4)
        output->Write(4, 1);    // absolute rendering intent
      } else {
        output->Write(2, 1);    // default to relative rendering intent
      }
    } else {
...
LLPrepare(JxlChunkedFrameInputSource input, size_t width, size_t height,
          ...
          int oneshot, JxlColorEncoding color_encoding) {
  assert(width != 0);
  assert(height != 0);
  ...
  frame_state->color_encoding = color_encoding;
...
JxlSimpleLosslessPrepareImpl(JxlChunkedFrameInputSource input, size_t width,
  ...
  if (bitdepth <= 8) {
    return LLPrepare(input, width, height, UpTo8Bits(bitdepth), nb_chans,
                     big_endian, effort, oneshot, color_encoding);
  }
  if (bitdepth <= 13) {
...
size_t JxlSimpleLosslessEncode(const unsigned char *rgba, size_t width,
  ...
  auto *frame_state = JxlSimpleLosslessPrepareFrame(
      ...
      effort, /*oneshot=*/true, color_encoding);
...
#if defined(_MSC_VER)
size_t Log2(uint32_t value) { return 31 - __lzcnt(value); }
#else
...
public:
  ...
  bool ParseUnsigned(size_t *number) {
    if (pos_ == end_)
      return error_msg("PNM: reached end before number");
    if (!IsDigit(*pos_))
      return error_msg("PNM: expected unsigned number");
  ...
  bool ParseSigned(double *number) {
    if (pos_ == end_)
      return error_msg("PNM: reached end before signed");
    ...
    if (is_neg || *pos_ == '+') {
      ++pos_;
      if (pos_ == end_)
        return error_msg("PNM: reached end before digits");
    }
...
private:
  ...
  bool SkipBlank() {
    if (pos_ == end_)
      return error_msg("PNM: reached end before blank");
  ...
  bool SkipSingleWhitespace() {
    if (pos_ == end_)
      return error_msg("PNM: reached end before whitespace");
    if (!IsWhitespace(*pos_))
      return error_msg("PNM: expected whitespace");
    ++pos_;
    return true;
  }
  ...
  bool MatchString(const char *keyword) {
    ...
    while (*kw) {
      if (ppos >= end_)
        return error_msg("PAM: unexpected end of input");
      if (*kw != *ppos)
        return false;
      ppos++;
      kw++;
...
int main(int argc, char **argv) {
  if (argc < 3) {
    fprintf(stderr,
            "Usage: %s in.ppm out.jxl [effort] [num_reps] [num_threads]\n",
            argv[0]);
    return 1;
  }
  ...
  size_t nb_chans, bitdepth, width, height;
  if (!DecodePAM(in, &image, &width, &height, &nb_chans, &bitdepth)) {
  ...
  for (size_t _ = 0; _ < num_reps; _++) {
    ...
    encoded_size = JxlSimpleLosslessEncode(
        image, width, stride, height, nb_chans, bitdepth,
        /*big_endian=*/true, effort, &encoded, &num_threads, +parallel_runner, color_encoding);
    if (encoded_size == 0)
      return EXIT_FAILURE;
  }
