...
bool CLR::Load()
{
	...
	//GetDll ?
	CHECK(pDefaultAppDomain->Load_2(_bstr_t("MathLib"), &pAssembly));

	//Get class inside namespace
	CHECK(pAssembly->CreateInstance(_bstr_t("MathLib.Math"), tmp.GetAddress()));
	_ObjectPtr pObject = (IUnknown*)tmp;

	_TypePtr pType = NULL; //args
