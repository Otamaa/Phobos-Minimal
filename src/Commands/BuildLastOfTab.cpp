...
void BuildLastOfTabCommandClass<TabIndex>::Execute(WWKey eInput) const
{
	...
	auto const pExt = HouseExtContainer::Instance.Find(pPlayer);

	auto const typeIndex = pExt->LastBuilt[TabIndex].PerTab;
	...
	auto const rtti = pExt->LastBuilt[TabIndex].RTTIPerTab;
	auto const isNaval = pExt->LastBuilt[TabIndex].IsNavalPerTab;

	EventClass ev(pPlayer->ArrayIndex,
		EventType::PRODUCE,
		rtti,
		typeIndex,
		isNaval ? TRUE : FALSE
