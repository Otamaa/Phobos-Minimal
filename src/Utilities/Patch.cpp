...
int GetSection(char* sectionName, LPVOID* pVirtualAddress)
{
	...
	for (int i = 0; i < pHeader->FileHeader.NumberOfSections; i++)
	{
		...
		{
			*pVirtualAddress = (LPVOID)((DWORD)hInstance + sct_hdr->VirtualAddress);
...
{
	auto pAddress = (LPVOID)pItem->offset;
	DWORD protect_flag;
...
void Patch::Apply(HANDLE process, const patch_decl* pItem)
{
	auto pAddress = (LPVOID)pItem->offset;
	DWORD protect_flag;
	VirtualProtectEx(process, pAddress, pItem->size, PAGE_READWRITE, &protect_flag);
	memcpy(pAddress, pItem->pData, pItem->size);
	VirtualProtectEx(process, pAddress, pItem->size, protect_flag, NULL);
}
