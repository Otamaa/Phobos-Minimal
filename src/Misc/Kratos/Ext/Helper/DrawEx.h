...
void DrawLaser(LaserType laser, CoordStruct sourcePos, CoordStruct targetPos, ColorStruct houseColor = ColorStruct::Empty);
void DrawLaser(CoordStruct sourcePos, CoordStruct targetPos, ColorStruct innerColor, ColorStruct outerColor = ColorStruct::Empty, int thickness = 2, int duration = 15);
...
namespace LaserHelper
{
	...
	void Cell(CoordStruct sourcePos, int length, ColorStruct lineColor, ColorStruct outerColor = ColorStruct::Empty, int thickness = 1, int duration = 1);
	void Crosshair(CoordStruct sourcePos, int length, ColorStruct lineColor, ColorStruct outerColor = ColorStruct::Empty, int thickness = 1, int duration = 1);
	...
	void MapCell(CoordStruct sourcePos, ColorStruct lineColor, ColorStruct outerColor = ColorStruct::Empty, int thickness = 1, int duration = 1, bool crosshair = false);
...
void DrawBeam(CoordStruct sourcePos, CoordStruct targetPos, BeamType type, ColorStruct customColor = ColorStruct::Empty);

void DrawRadBeam(CoordStruct sourcePos, CoordStruct targetPos, ColorStruct customColor = ColorStruct::Empty, int period = 15, double amplitude = 40.0);
void DrawTemporalBeam(CoordStruct sourcePos, CoordStruct targetPos, ColorStruct customColor = ColorStruct::Empty, int period = 15, double amplitude = 40.0);
#pragma endregion
