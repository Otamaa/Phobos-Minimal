...
int GetRealDamage(Armor armor, int damage, WarheadTypeClass* pWH, bool ignoreArmor, int distance)
{
	...
	if (!ignoreArmor)
	{
		...
		if (realDamage > 0)
		{
			realDamage = FakeWarheadTypeClass::ModifyDamageA(damage, (FakeWarheadTypeClass*)pWH, armor, distance);
		}
		else
		{
			realDamage = -FakeWarheadTypeClass::ModifyDamageA(-damage, (FakeWarheadTypeClass*)pWH, armor, distance);
...
bool CanDamageMe(TechnoClass* pTechno, int damage, int distanceFromEpicenter, WarheadTypeClass* pWH, int& realDamage, bool effectsRequireDamage)
{
	...
	else
	{
		...
		{
			// 必须要可以造成伤害
			if (FakeWarheadTypeClass::ModifyDamageA(RulesClass::Instance->MaxDamage, (FakeWarheadTypeClass*)pWH, pTechno->GetType()->Armor, 0) == 0)
			{
				// 弹头无法对该类型护甲造成伤害
				return false;
			}
...
void ForceStopMoving(FootClass* pFoot)
{
	...
	pFoot->ArchiveTarget = nullptr;
	// 清除寻路目的地
	ILocomotion* pLoco = pFoot->Locomotor.GetInterfacePtr();
	pLoco->Mark_All_Occupation_Bits((int)MarkType::Remove); // 清除HeadTo的占领
...
{
	pLoco->Stop_Moving();
	pLoco->Mark_All_Occupation_Bits((int)MarkType::Remove);
