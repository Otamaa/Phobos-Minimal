...
{
	FootClass* pTargetFoot = flag_cast_to<FootClass*>(pTarget);
...
void JumpjetCarryall::DropPayload()
{
	...
	if (pTarget)
	{
		...
		if (TryPutTechno(pTarget, location))
		{
			...
			if (sound != -1)
			{
				VocClass::ImmedietelyPlayAt(sound, location, nullptr);
			}
		}
		else
		{
...
bool JumpjetCarryall::NotToTarget()
{
	...
	{
		if (AbstractClass* pDest = flag_cast_to<FootClass*, true>(pTechno)->Destination)
		{
