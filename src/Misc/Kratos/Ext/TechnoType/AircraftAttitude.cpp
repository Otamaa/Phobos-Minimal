...
bool AircraftAttitude::TryGetAirportDir(int& poseDir)
{
	...
	AircraftClass* pAir = cast_to<AircraftClass*, true>(pTechno);
	if (pAir->HasAnyLink()) // InRadioContact
	{
		TechnoClass* pAirport = pAir->GetNthLink();
		// 傻逼飞机是几号停机位
		int index = pAirport->FindLinkIndex(pAir);
		if (index < 128)
		{
...
void AircraftAttitude::UpdateHeadToCoord(CoordStruct headTo, bool lockAngle)
{
	...
	if (!headTo.IsEmpty())
	{
		...
		if (Math::abs(z) <= Unsorted::LevelHeight)
		{
			...
			if (pCell && pNextCell && pNextCell != pCell)
			{
				...
				int zCell = Math::abs(pos1.Z - pos2.Z);
...
void AircraftAttitude::OnUpdate()
{
	...
	if (!IsDeadOrInvisible(pTechno))
	{
		...
		// 正事
		// 检查是否开启了吊运功能
		if (cast_to<AircraftClass*, true>(pTechno)->Type->Carryall)
		{
			PitchAngle = 0;
		}
		else
		{
			// 角度差比Step大
			if (_smooth && PitchAngle != _targetAngle && Math::abs(_targetAngle - PitchAngle) > angelStep)
			{
				// 只调整一个step
