...
void TechnoStatus::DrawSHP_Paintball_BuildingAnim(REGISTERS* R)
{
	...
	if (!_airstrikes.empty())
	{
		...
		{
			R->EBP(airstrikeData->AirstrikeLineColor.ToInit());
		}
	}
	if (pTechno->Berzerk)
	{
...
void TechnoStatus::DrawSHP_Colour(REGISTERS* R)
{
	if (!IsDead(pTechno) && !pTechno->IsVoxel())
	{
		...
		if (!_airstrikes.empty())
		{
			...
			{
				R->EAX(airstrikeData->AirstrikeLineColor.ToInit());
			}
		}
		// Berzerk, 建筑不支持狂暴染色
...
void TechnoStatus::DrawVXL_Paintball(REGISTERS* R, bool isBuilding)
{
	if (isBuilding)
	{
		...
		if (!_airstrikes.empty())
		{
			...
			{
				R->Stack(0x24, airstrikeData->AirstrikeLineColor.ToInit());
			}
		}
	...
	else
	{
		if (!_airstrikes.empty())
		{
			...
			{
				R->ESI(airstrikeData->AirstrikeLineColor.ToInit());
			}
		}
	}
