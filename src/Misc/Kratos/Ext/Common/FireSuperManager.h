...
#include <Misc/Kratos/Ext/EffectType/Effect/FireSuperData.h>
...
public:
	...
	public:
		...
		HouseClass* GetHouse()
		{
			return HouseClass::Array->get_or_default(HouseIndex);
		}
...
#pragma region Save/Load
	static void SaveSuperQueue(EventSystem* sender, Event e, void* args)
	{
		...
		if (arg->IsStartInStream())
		{
			...
			saver.WriteToStream(arg->Stream);
		}
	}
	static void LoadSuperQueue(EventSystem* sender, Event e, void* args)
	{
		...
		if (arg->IsStartInStream())
		{
			...
			loader.ReadFromStream(arg->Stream);
