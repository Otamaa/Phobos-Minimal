...
void StandEffect::CreateAndPutStand()
{
	...
	if (pType)
	{
		pStand = flag_cast_to<TechnoClass*, true>(pType->CreateObject(AE->pSourceHouse));
	}
	if (pStand)
	{
		// 初始化替身的状态设置
...
void StandEffect::ExplodesOrDisappear(bool peaceful)
{
	...
	if (pTemp)
	{
		...
		if (TryGetStatus<TechnoExt>(pTemp, standStatus))
		{
			...
			if ((masterIsRocket || pTemp->InLimbo) && !KratosCommon::IsScenarioClear)
			{
...
void StandEffect::UpdateStateTechno(bool masterIsDead)
{
	...
	if (IsBuilding())
	{
		...
		{
			pStand->ArchiveTarget = pTechno->ArchiveTarget;
		}
	}
	...
	{
		FootClass* pFoot = flag_cast_to<FootClass*, true>(pStand);
		...
		if (!Data->IsTrain)
		{
			if (Data->SameTilter)
			{
				...
				if (Data->SameTilter)
				{
					...
					// 同步 替身 与 JOJO 的地形角度
					ILocomotion* masterLoco = flag_cast_to<FootClass*, true>(pTechno)->Locomotor.GetInterfacePtr();
					ILocomotion* standLoco = flag_cast_to<FootClass*, true>(pStand)->Locomotor.GetInterfacePtr();

					DWORD previousRamp = 0;
					DWORD currentRamp = 0;

					if (DriveLocomotionClass* pMasterDriveLoco = locomotion_cast<DriveLocomotionClass*>(masterLoco))
					{
						previousRamp = pMasterDriveLoco->PreviousRamp;
						currentRamp = pMasterDriveLoco->CurrentRamp;
					}
					else if (ShipLocomotionClass* pMasterShipLoco = locomotion_cast<ShipLocomotionClass*>(masterLoco))
					{
						previousRamp = pMasterShipLoco->PreviousRamp;
						currentRamp = pMasterShipLoco->CurrentRamp;
					}

					if (DriveLocomotionClass* pStandDriveLoco = locomotion_cast<DriveLocomotionClass*>(standLoco))
					{
						pStandDriveLoco->PreviousRamp = previousRamp;
						pStandDriveLoco->CurrentRamp = currentRamp;
					}
					else if (ShipLocomotionClass* pStandShipLoco = locomotion_cast<ShipLocomotionClass*>(standLoco))
					{
