...
{
	// 不允许朝这个目标发射
	if (!weaponTypeData.CanFireToTarget(pTarget, flag_cast_to<ObjectClass*>(pObject), pAttacker, pAttackingHouse, pWeapon))
	{
		return false;
	}
	...
	else if (weaponTypeData.CheckAAOnly && pWeapon->Projectile->AA)
	{
		...
		if (pTechnoType && pTechnoType->LandTargeting != LandTargetingType::Land_not_okay)
		{
	...
	if (!weaponTypeData.CheckRange || InRange(pObject, pTarget, pWeapon, minRange, maxRange))
	{
		...
		else
		{
			...
			for (int i = 0; i < burst; i++)
			{
				...
				// 发射武器，全射出去
				BulletClass* pBullet = FireBulletTo(flag_cast_to<ObjectClass*>(pObject), pAttacker, pTarget, pAttackingHouse, pWeapon, sourcePos, targetPos, bulletVelocity, flh, !isOnBody);
				// 记录下子机发射器的开火坐标
				if (pWeapon->Spawner)
				{
...
void AttachFire::SimulateBurstFireOnce(SimulateBurst& burst)
{
	...
	VelocityClass bulletVelocity = VelocityClass::Empty;
	// 扇形攻击
	...
	BulletClass* pBullet = FireBulletTo(flag_cast_to<ObjectClass*>(pObject), burst.pAttacker, burst.pTarget, burst.pAttackingHouse, pWeapon, sourcePos, targetPos, bulletVelocity, tempFLH, !burst.IsOnBody);
	// 记录下子机发射器的开火坐标
	if (pWeapon->Spawner)
	{
