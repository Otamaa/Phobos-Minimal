...
CoordStruct TeleportState::GetAndMarkDestination(CoordStruct location)
{
	...
	if (!pFoot->IsAttackedByLocomotor)
	{
		...
		else
		{
			...
			targetPos = pFoot->Locomotor->Destination();
...
void TeleportState::OnUpdate()
{
	if (!IsBuilding() && !IsDeadOrInvisible(pTechno))
	{
				...
				if (!targetPos.IsEmpty())
				{
					...
					if (!Data.Offset.IsEmpty())
					{
						...
						if (Data.IsOnTarget && CastToTechno(pTarget, pTargetTechno))
						{
							...
							else if (pTargetTechno->GetTechnoType()->Locomotor == JumpjetLocomotionClass::ClassGUID())
							{
								FootClass* pTargetFoot = flag_cast_to<FootClass*, true>(pTargetTechno);
								if (JumpjetLocomotionClass* jjLoco = locomotion_cast<JumpjetLocomotionClass*>(pTargetFoot->Locomotor))
								{
									facing = jjLoco->Facing.Current();
								}
							}
							else
							{
					...
					// 检查目的地是否可以着陆
					bool isJJ = pTechno->GetTechnoType()->Locomotor == JumpjetLocomotionClass::ClassGUID();
					...
					if (CellClass* pCell = MapClass::Instance->TryGetCellAt(targetPos))
					{
						...
						else
						{
							...
							do
							{
								...
								Move move = pTechno->IsCellOccupied(pCell, FacingType::None, -1, nullptr, false);
								switch (move)
								{
								case Move::OK:
								...
								ZoneType zone = MapClass::Instance->GetMovementZoneType(curretCell, pType->MovementZone, pTechno->IsOnBridge());
					...
					if (pTargetCell)
					{
						...
						if (pTechno->IsInAir() || pTechno->WhatAmI() == AbstractType::Aircraft)
						{
							...
							// 移动位置
							pTechno->Mark(MarkType::Remove);
							pTechno->SetLocation(targetPos);
							pTechno->Mark(MarkType::Put);
							if (isJJ)
							{
								pFoot->Jumpjet_OccupyCell(pTargetCell->MapCoords);
							}
							// 设置面向
							pTechno->PrimaryFacing.Set_Current(pTechno->PrimaryFacing.Current());
							pTechno->SecondaryFacing.Set_Current(pTechno->SecondaryFacing.Current());
							// 移除黑幕
							...
							if (Data.FreezingInAir)
							{
								...
								if (typeData->ChronoTrigger)
								{
									...
									{
										int factor = MaxImpl(typeData->ChronoDistanceFactor, 1);
						...
						else
						{
							...
							pFoot->Locomotor->Force_Track(-1, location);
							pFoot->FrozenStill = true;
							if (!locomotion_cast<TeleportLocomotionClass*>(pFoot->Locomotor.GetInterfacePtr()))
							{
								LocomotionClass::ChangeLocomotorTo(pFoot, TeleportLocomotionClass::ClassGUID());
							}
							// 移动到目的地
		...
		case TeleportStep::MOVEFORWARD:
		{
			...
			{
				FootClass* pFoot = flag_cast_to<FootClass*, false>(pTechno);
				AircraftTrackerClass::Instance->Update(pFoot, pFoot->LastFlightMapCoords, pCell->MapCoords);
			}
			if (!pTechno->Target)
			{
				if (Data.MoveForward)
