...
#include <Misc/Kratos/Ext/TechnoType/DecoyMissile.h>


void KratosMissileTrajectory::CheckTargetHasDecoy()
{
...
BulletStatus* KratosMissileTrajectory::GetBulletStatus()
{
	if (!_status)
	{
...
void KratosMissileTrajectory::Awake()
{
	if (!IsMissile())
	{
		Disable();
...
void KratosMissileTrajectory::Destroy()
{
...
void KratosMissileTrajectory::OnPut(CoordStruct* pCoord, DirType dirType)
{
	// 高抛导弹
...
// 热诱弹
void KratosMissileTrajectory::OnUpdate()
{
	// 是否是对飞行器攻击
	CheckTargetHasDecoy();
	if (IsDecoy)
