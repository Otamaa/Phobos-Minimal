...
#include <Misc/Kratos/Ext/WeaponType/ProximityRangeData.h>
			...
			if (pSourceCell != _proximity.pCheckedCell)
			{
				...
				for (TechnoClass* pTarget : pTechnoList)
				{
					if (!IsDeadOrInvisible(pTarget))
					{
						...
						{
							if (BuildingClass* pBuilding = cast_to<BuildingClass*, true>(pTarget))
							{
								// 检查建筑在范围内
...
bool BulletStatus::ManualDetonation(CoordStruct sourcePos, bool KABOOM, AbstractClass* pTarget, CoordStruct detonatePos)
{
	...
	else if (pTarget)
	{
		...
		// 在预定引爆地点引爆弹头
		DamageArea::Apply(&detonatePos, damage, pBullet->Owner, pWH, pWH->Tiberium, pSourceHouse);
		// 播放弹头动画
