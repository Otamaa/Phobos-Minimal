...
#include <Misc/Kratos/Ext/TechnoType/DisguiseData.h>
#include <Misc/Kratos/Ext/TechnoType/JumpjetCarryall.h>
#include <Misc/Kratos/Ext/TechnoType/SHPFVTurretData.h>
...
ASMJIT_PATCH(0x6FF923, TechnoClass_DeployFire_FireOnce, 0x6)
{
	GET(TechnoClass*, pTechno, ESI);
...
// ASMJIT_PATCH(0x739C7A, UnitClass_SimpleDeployer_Deployed, 0x6)
// {
// 	GET(TechnoClass*, pTechno, ESI);
// 	if (TechnoStatus* status = GetStatus<TechnoExt, TechnoStatus>(pTechno))
// 	{
...
ASMJIT_PATCH(0x7466D8, UnitClass_Set_Disguise, 0xA)
{
	GET(AbstractClass*, pTarget, ESI);
	if (flag_cast_to<ObjectClass*, true>(pTarget)->IsDisguised())
	{
		// 伪装成目标单位的伪装
		return 0x7466E6;
	}
	...
	{
		// 我自己来
		TechnoClass* pTargetTechno = flag_cast_to<TechnoClass*, true>(pTarget);
...
ASMJIT_PATCH(0x746B6D, UnitClass_Disguise_FullName, 0x5)
{
	GET(TechnoClass*, pTechno, ESI);
...
ASMJIT_PATCH(0x736A26, UnitClass_Rotation_SetTurretFacingToTarget_Skip, 0x6)
{
	GET(TechnoClass*, pTechno, ESI);
...
ASMJIT_PATCH(0x736BCA, UnitClass_Rotation_SetTurretFacing_NoTargetAndStanding, 0x5)
{
	GET(TechnoClass*, pTechno, ESI);
...
ASMJIT_PATCH(0x736BBB, UnitClass_Rotation_SetTurretFacing_NoTargetAndMoving, 0x5)
{
	GET(TechnoClass*, pTechno, ESI);
...
ASMJIT_PATCH(0x736B7E, UnitClass_Rotation_SetTurretFacing_Skip, 0xA)
{
	GET(TechnoClass*, pTechno, ESI);
...
ASMJIT_PATCH(0x73CD01, UnitClass_Draw_ChangeTurret_SHPIFV, 0x5)
{
...
ASMJIT_PATCH(0x73C71D, UnitClass_DrawSHP_FacingDir, 0x6)
{
	...
	{
		// WWSB 自己算起始帧
		if (UnitTypeClass* pTargetType = type_cast<UnitTypeClass*>(pTechno->GetDisguise(true)))
		{
...
ASMJIT_PATCH(0x73C725, UnitClass_DrawSHP_HasTurret, 0x6)
{
	GET(TechnoClass*, pTechno, EBP);
	if (pTechno->IsDisguised() && !pTechno->IsClearlyVisibleTo(HouseClass::CurrentPlayer))
	{
		ObjectTypeClass* pTargetType = pTechno->GetDisguise(true);
		auto const pTargetTechnoType = type_cast<TechnoTypeClass*>(pTargetType);
...
ASMJIT_PATCH(0x73BDA3, UnitClass_DrawVoxel_TurretFacing, 0x5)
{
	...
	if (!pTechno->GetTechnoType()->Turret && pTechno->IsDisguised() && !pTechno->IsClearlyVisibleTo(HouseClass::CurrentPlayer))
	{
		// 本体没有炮塔
		ObjectTypeClass* pTargetType = pTechno->GetDisguise(true);
		if (pTargetType && type_cast<TechnoTypeClass*>(pTargetType)->Turret)
		{
			// 伪装的对象有炮塔，将身体的朝向赋给炮塔
...
ASMJIT_PATCH(0x73B8DB, UnitClass_DrawVoxel_HasChargeTurret, 0x8)
{
	GET(TechnoClass*, pTechno, EBP);
...
ASMJIT_PATCH(0x73BC1D, UnitClass_DrawVoxel_HasChargeTurret2, 0x7)
{
	GET(TechnoClass*, pTechno, EBP);
	GET(TechnoTypeClass*, pTechnoType, EBX);
	// 渲染伪装对象
...
ASMJIT_PATCH(0x706724, TechnoClass_Draw_VXL_Disguise_Blit_Flags, 0x5)
{
	return 0x706731;
}
#pragma endregion
...
ASMJIT_PATCH(0x73D6F8, UnitClass_Mission_Unload_Transporter, 0x6)
{
	GET(TechnoClass*, pTechno, ESI);
...
// ASMJIT_PATCH(0x73D6E6, UnitClass_Mission_Unload_Carryall_Drop, 0x6) //phobos skipped this address
ASMJIT_PATCH(0x73D6EC, UnitClass_Mission_Unload_Carryall_Drop, 0x6)
{
	GET(TechnoClass*, pTechno, ESI);
	JumpjetCarryall* carry = GetScript<TechnoExt, JumpjetCarryall>(pTechno);
	if (carry && carry->pPayload)
...
ASMJIT_PATCH(0x74041B, UnitClass_WhatAction_Carryall_Lifting, 0x5)
{
...
ASMJIT_PATCH(0x74000B, UnitClass_WhatAction_Carryall_Droping, 0x6)
{
	GET(TechnoClass*, pTechno, ESI);
	JumpjetCarryall* carry = GetScript<TechnoExt, JumpjetCarryall>(pTechno);
	if (carry && carry->pPayload)
...
ASMJIT_PATCH(0x73FFF4, UnitClass_WhatAction_TransportHover, 0x8)
{
	GET(TechnoClass*, pTechno, ESI);
	if (pTechno->GetTechnoType()->BalloonHover && pTechno->IsInAir())
	{
...
ASMJIT_PATCH(0x7388FD, UnitClass_ActionClick_Carryall, 0x5)
{
	...
	{
		GET(TechnoClass*, pTarget, EDI);
		if (FootClass* pTargetFoot = flag_cast_to<FootClass*>(pTarget))
		{
...
ASMJIT_PATCH(0x73CF16, UnitClass_Draw_It_Carryall_HightOffset, 0x7)
{
	GET(TechnoClass*, pTechno, ESI);
...
ASMJIT_PATCH(0x73D317, UnitClass_Draw_It, 0x6)
{
	GET(TechnoClass*, pTechno, ESI);
...
// 被捕获时切换图像
// ASMJIT_PATCH(0x4DB157, FootClass_DrawVoxel_Shadow_Carryall, 0x8)
// {
