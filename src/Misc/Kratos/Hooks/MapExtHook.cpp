			...
			if (WarheadTypeExt::TypeData* warheadTypeData = GetTypeData<WarheadTypeExt, WarheadTypeExt::TypeData>(pWH))
			{
				...
				if (money != 0)
				{
					...
					if (warheadTypeData->ShowMeTheMoneyDisplay)
					{
						...
						if (money < 0)
						{
							text.append(L"-$").append(std::to_wstring(Math::abs(money)));
							textData.Setup(true);
						}
						else
						{
...
ASMJIT_PATCH(0x481D0E, CellClass_CrateBeingCollected_Firepower1, 0x6)
{
...
ASMJIT_PATCH(0x481C6C, CellClass_CrateBeingCollected_Armor1, 0x6)
{
	GET(TechnoClass*, pTechno, EDI);
	if (TechnoStatus* status = GetStatus<TechnoExt, TechnoStatus>(pTechno))
	{
...
ASMJIT_PATCH(0x481CE1, CellClass_CrateBeingCollected_Speed1, 0x6)
{
	GET(TechnoClass*, pTechno, EDI);
	if (TechnoStatus* status = GetStatus<TechnoExt, TechnoStatus>(pTechno))
	{
...
ASMJIT_PATCH(0x481D3D, CellClass_CrateBeingCollected_Cloak1, 0x6)
{
	GET(TechnoClass*, pTechno, EDI);
	if (TechnoStatus* status = GetStatus<TechnoExt, TechnoStatus>(pTechno))
	{
...
ASMJIT_PATCH(0x483226, CellClass_CrateBeingCollected_Firepower2, 0x6)
{
...
ASMJIT_PATCH(0x482E57, CellClass_CrateBeingCollected_Armor2, 0x6)
{
	GET(TechnoClass*, pTechno, ECX);
...
ASMJIT_PATCH(0x48303A, CellClass_CrateBeingCollected_Speed2, 0x6)
{
	GET(TechnoClass*, pTechno, ECX);
...
ASMJIT_PATCH(0x48294F, CellClass_CrateBeingCollected_Cloak2, 6)
{
...
ASMJIT_PATCH(0x69251A, ScrollClass_ProcessClickCoords_VirtualUnit, 0x6)
{
...
ASMJIT_PATCH(0x6DA3FF, TacticalClass_SelectAt_VirtualUnit, 0x6)
{
	GET(AbstractClass*, pTarget, EAX);
	if (pTarget)
	{
...
ASMJIT_PATCH(0x4A904E, DisplayClass_Passes_Proximity_Check_MobileMCV, 0x5)
{
