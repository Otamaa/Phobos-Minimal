...
// AI的替身在攻击友军时会强制取消目标
ASMJIT_PATCH(0x6FA45D, TechnoClass_Update_NotHuman_ClearTarget_Stand, 0x6)
{
	GET(TechnoClass*, pTechno, ESI);
...
ASMJIT_PATCH(0x5F6B90, ObjectClass_InAir_SkipCheckIsOnMap, 0x5)
{
...
ASMJIT_PATCH(0x4D94B0, FootClass_SetDestination_Stand, 0x5)
{
...
// Stand support Radar
ASMJIT_PATCH(0x508EC6, HouseClass_RadarUpdate_Stand, 0x5)
{
...
ASMJIT_PATCH(0x508F9B, HouseClass_SpySatUpdate_Stand, 0x5)
{
	GET(TechnoClass*, pTechno, ECX);
	if (pTechno->IsOnMap || AmIStand(pTechno))
	{
...
ASMJIT_PATCH(0x6FC749, TechnoClass_CanFire_WhichLayer_Stand, 0x5)
{
	GET(Layer, layer, EAX);
...
ASMJIT_PATCH(0x6FF66C, TechnoClass_Fire_DecreaseAmmo, 0x6)
{
	GET(TechnoClass*, pTechno, ESI);
	TechnoStatus* status = nullptr;
...
ASMJIT_PATCH(0x6FCDBE, TechnoClass_SetTarget_Stand, 0x6)
{
...
ASMJIT_PATCH(0x4D9947, FootClass_Greatest_Threat_GetTarget, 0x6)
{
	GET(TechnoClass*, pTechno, ESI);
...
ASMJIT_PATCH(0x55DBC3, MainLoop_ShortLayerClass, 0x5)
{
	DisplayClass::GetLayer(Layer::Air)->Sort();
	return 0;
}

ASMJIT_PATCH(0x4DA87A, FootClass_Update_UpdateLayer, 0x6)
{
	GET(TechnoClass*, pTechno, ESI);
...
ASMJIT_PATCH(0x4DB091, FootClass_GetZAdjustment, 0x6)
{
	GET(TechnoClass*, pTechno, ESI);
	...
	else
	{
		...
		if (TryGetStatus<TechnoExt>(pTechno, status) && status->AmIStand() && status->pMyMaster && !status->MyStandData.IsTrain)
		{
			...
			if (BuildingClass* pBuilding = cast_to<BuildingClass*, true>(pMaster))
			{
...
ASMJIT_PATCH(0x5F6BF7, ObjectClass_GetYSort, 0x5)
{
	...
	else if (TechnoClass* pTechno = flag_cast_to<TechnoClass*>(pObject))
	{
		TechnoStatus* status = nullptr;
		if (TryGetStatus<TechnoExt, TechnoStatus>(pTechno, status) && status->AmIStand() && status->pMyMaster && !status->MyStandData.IsTrain)
		{
			...
			if (offset >= 0)
			{
				if (BuildingClass* pBuilding = cast_to<BuildingClass*>(status->pMyMaster))
				{
					BuildingTypeClass* pType = pBuilding->Type;
					int bY = 0;
					for (int i = 0; i < 0x15; i++)
...
ASMJIT_PATCH(0x75C7E0, WalkLocomotionClass_In_Which_Layer, 0x5) // WalkLoco
{
	GET(TechnoClass*, pTechno, ESI);
...
ASMJIT_PATCH(0x6A3E50, ShipLocomotionClass_In_Which_Layer, 0x5) // ShipLoco
{
	GET(TechnoClass*, pTechno, ESI);
	Layer layer = StandLayer::GetLayer(pTechno);
	R->EAX(layer);
...
ASMJIT_PATCH(0x5B19D0, MechLocomotionClass_In_Which_Layer, 0x5) // MechLoco
{
	GET(TechnoClass*, pTechno, ESI);
	Layer layer = StandLayer::GetLayer(pTechno);
	R->EAX(layer);
...
ASMJIT_PATCH(0x517100, HoverLocomotionClass_In_Which_Layer, 0x5) // HoverLoco
{
	GET(TechnoClass*, pTechno, ESI);
	Layer layer = StandLayer::GetLayer(pTechno);
	R->EAX(layer);
...
ASMJIT_PATCH(0x4B4820, DriveLocomotionClass_In_Which_Layer, 0x5) // DriveLoco
{
	GET(TechnoClass*, pTechno, ESI);
	Layer layer = StandLayer::GetLayer(pTechno);
	R->EAX(layer);
...
ASMJIT_PATCH(0x414C27, AircraftClass_Update_SkipCreateChronoSparkleAnimOnStand, 0x5)
{
	GET(TechnoClass*, pTechno, ESI);
...
ASMJIT_PATCH(0x440406, BuildingClass_Update_SkipCreateChronoSparkleAnimOnStand1, 0x5)
{
	GET(TechnoClass*, pTechno, ESI);
...
ASMJIT_PATCH(0x44050C, BuildingClass_Update_SkipCreateChronoSparkleAnimOnStand2, 0x5)
{
	GET(TechnoClass*, pTechno, ESI);
...
ASMJIT_PATCH(0x51BB17, InfantryClass_Update_SkipCreateChronoSparkleAnimOnStand, 0x5)
{
	GET(TechnoClass*, pTechno, ESI);
...
ASMJIT_PATCH(0x736250, UnitClass_Update_SkipCreateChronoSparkleAnimOnStand, 0x5)
{
	GET(TechnoClass*, pTechno, ESI);
