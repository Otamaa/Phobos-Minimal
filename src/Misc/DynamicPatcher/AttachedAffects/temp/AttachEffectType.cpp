...
void AttachEffectType::LoadFromINI(CCINIClass* pINI)
{
	...
	AffectTypes.Read(exINI, pSection, "AffectTypes", true);
	NotAffectTypes.Read(exINI, pSection, "NotAffectTypes", true);
		...
		if (exINI.ReadString(pSection, pNumKey))
		{
				...
				cur = strtok_s(nullptr, Phobos::readDelims, &context))
			{
				if (Phobos::Config::MoreDetailSLDebugLog)
					Debug::LogInfo("Parsing {} Size of [{}]{}={} idx[{}] ", nBaseSize, pSection, pNumKey, cur, nCount);
