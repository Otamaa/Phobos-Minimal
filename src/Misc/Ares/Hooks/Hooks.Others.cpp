...
DEFINE_OVERRIDE_HOOK(0x489235, GetTotalDamage_Verses, 0x8)
{
	...
	const auto vsData = &pExt->Verses.at(Armor);
...
DEFINE_OVERRIDE_HOOK(0x6F7D3D, TechnoClass_CanAutoTargetObject_Verses, 0x7)
{
	...
	const auto vsData = &pData->Verses.at(Armor);

	return vsData->PassiveAcquire || !(vsData->Verses <= 0.02)
		? ContinueCheck
		: ReturnFalse
		;
}
...
DEFINE_OVERRIDE_HOOK(0x6FCB6A, TechnoClass_CanFire_Verses, 0x7)
{
	...
	return vsData->ForceFire || !(vsData->Verses == 0.0)
		? ContinueCheck
		: FireIllegal
		;
}
...
DEFINE_OVERRIDE_HOOK(0x70CEA0, TechnoClass_EvalThreatRating_TargetWeaponWarhead_Verses, 0x6)
{
	...
	const auto pData = WarheadTypeExt::ExtMap.Find(pWH);
	const auto vsData = &pData->Verses.at((int)pThisType->Armor);
	double nMult = 0.0;
...
DEFINE_OVERRIDE_HOOK(0x70CF45, TechnoClass_EvalThreatRating_ThisWeaponWarhead_Verses, 0xB)
{
	...
	const auto pData = WarheadTypeExt::ExtMap.Find(pWeapon->Warhead);
	const auto vsData = &pData->Verses.at(Armor);
...
DEFINE_OVERRIDE_HOOK(0x6F36E3 , TechnoClass_SelectWeapon_Verses, 0x5)
{
	...
	const int nArmot = (int)pTarget->GetTechnoType()->Armor;
	const auto pWH_A_Ext = WarheadTypeExt::ExtMap.Find(pWeapon_A->Warhead);
	const auto vsData_A = &pWH_A_Ext->Verses.at(nArmot);

	if (vsData_A->Verses == 0.0)
		return UsePrimary;

	const auto pWH_B_Ext = WarheadTypeExt::ExtMap.Find(pWeapon_B->Warhead);
	const auto vsData_B = &pWH_B_Ext->Verses.at(nArmot);

	if (vsData_B->Verses != 0.0)
...
DEFINE_OVERRIDE_HOOK(0x708AF7, TechnoClass_ShouldRetaliate_Verses, 0x7)
{
	...
	return vsData->Retaliate || !(vsData->Verses <= 0.0099999998)
		? Retaliate
		: DoNotRetaliate
		;
}
