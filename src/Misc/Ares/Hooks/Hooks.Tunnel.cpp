...
//{
//	const auto pTunnelTypeExt = BuildingTypeExt::ExtMap.Find(pTunnel->Type);
//
...
{
	if (pTunnelData->begin() == pTunnelData->end())
		return;
	...
	const auto It = std::find_if_not(pOwner->Buildings.begin(), pOwner->Buildings.end(), [pTunnel](BuildingClass* pBld)
 {
	 if (pBld->Health > 0 && !pBld->InLimbo && pBld->IsOnMap)
	 {
		 ...
		 {
			 if (TunnelIdx(pBld->Type) == TunnelIdx(pTunnel->Type))
			 {
				 return false;
			 }
		 }
...
//		return Nothing;
//
//	const auto pTunnelData = AresData::GetTunnelArray(pTarget->Type, pTarget->Owner);
//	if (!pTunnelData)
//		return Nothing;
//
//	return CanEnterTunnel(pTunnelData, pTarget, pThis) ?
...
//
//	if (const auto pTunnelData = AresData::GetTunnelArray(pTarget->Type, pTarget->Owner))
//		DestroyTunnel(pTunnelData, pTarget, pThis->Owner);
//
//	return 0;
//}
//
...
//
//	if (const auto pTunnelData = AresData::GetTunnelArray(pTarget->Type, pTarget->Owner)){
//		DestroyTunnel(pTunnelData, pTarget, generic_cast<TechnoClass*>(pKiller));
//	}
//
//	return 0;
//}
...
DEFINE_OVERRIDE_HOOK(0x73F606, UnitClass_IsCellOccupied_Tunnel, 0x6)
{
	...
	if (!canbeDestination)
		canbeDestination = TunnelIdx(pBuilding->Type) >= 0;
