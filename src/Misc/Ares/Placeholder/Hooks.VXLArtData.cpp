...
DEFINE_OVERRIDE_HOOK(0x73B90E, UnitClass_DrawVXL_Barrels1, 7)
{
	...
	R->Stack(0x2C, R->ESI());
	const auto pData = AresData::GetBarrelsVoxel(pUnit, nIdx);
...
DEFINE_OVERRIDE_HOOK(0x73BCCD, UnitClass_DrawVXL_Barrels2, 7)
{
	GET(UnitTypeClass*, pUnit, EBX);
	GET(int, nIdx, ECX);
	R->EDX(AresData::GetBarrelsVoxel(pUnit, nIdx));
	return 0x73BCD4;
}
...
DEFINE_OVERRIDE_HOOK(0x73BD6A, UnitClass_DrawVXL_Barrels3, 7)
{
	GET(UnitTypeClass*, pUnit, EBX);
	GET(int, nIdx, ESI);
	R->ECX(AresData::GetBarrelsVoxel(pUnit, nIdx));
	return 0x73BD71;
}
...
DEFINE_OVERRIDE_HOOK(0x73BD15, UnitClass_DrawVXL_Turrets, 7)
{
	GET(UnitTypeClass*, pUnit, EBX);
	GET(int, nIdx, ESI);
	R->ECX(AresData::GetTurretsVoxel(pUnit, nIdx));
	return 0x73BD1C;
}
