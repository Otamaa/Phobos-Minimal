...
public:
	...
	void Submit_Key(void const* key, int length)
	{
		...
		for (int index = 0; index < (int)Defines::ROUNDS + 2; index++)
		{
		...
		unsigned long* p_de = &P_Decrypt[(int)Defines::ROUNDS + 1];
		for (int p_index = 0; p_index < (int)Defines::ROUNDS + 2; p_index += 2)
		{
	...
	int Encrypt(void const* plaintext, int length, void* cyphertext)
	{
		...
		if (IsKeyed)
		{
			...
			for (int index = 0; index < blocks; index++)
			{
				...
				plaintext = ((char*)plaintext) + (int)Defines::BYTES_PER_BLOCK;
				cyphertext = ((char*)cyphertext) + (int)Defines::BYTES_PER_BLOCK;
			}
			int encrypted = blocks * (int)Defines::BYTES_PER_BLOCK;
...
private:
	...
	constexpr void Sub_Key_Encrypt(unsigned long& left, unsigned long& right)
	{
		...
		for (int index = 0; index < (int)Defines::ROUNDS; index += 2)
		{
		...
		left = r.Long ^ P_Encrypt[(int)Defines::ROUNDS + 1];
...
private:
	...
	unsigned long P_Encrypt[(int)Defines::ROUNDS + 2];
	unsigned long P_Decrypt[(int)Defines::ROUNDS + 2];
...
public:
	...
	virtual int Put(const void* source, int slen) override
	{
		...
		if (Counter)
		{
			...
			{
				if (Control == CryptControl::DECRYPT)
				{
