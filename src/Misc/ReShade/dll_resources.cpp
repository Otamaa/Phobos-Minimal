...
{
	const HRSRC info = FindResource(ReShadeIntegration::g_module_handle, MAKEINTRESOURCE(id), RT_RCDATA);
	assert(info != nullptr);
	const HGLOBAL handle = LoadResource(ReShadeIntegration::g_module_handle, info);

	data_resource result;
	result.data = LockResource(handle);
	result.data_size = SizeofResource(ReShadeIntegration::g_module_handle, info);
...
{
	LPCWSTR s = nullptr;
	const int length = LoadStringW(ReShadeIntegration::g_module_handle, id, reinterpret_cast<LPWSTR>(&s), 0);
	assert(length > 0);
...
std::vector<std::string> reshade::resources::get_languages()
{
	...
	EnumResourceNamesW(ReShadeIntegration::g_module_handle, RT_STRING,
	...
	EnumResourceLanguages(ReShadeIntegration::g_module_handle, RT_STRING, first_string_table_block,
