	...
	_enabled_features(enabled_features)
{
	...
	{	constexpr VkDescriptorPoolSize pool_sizes[] = {
		...
		{
			Debug::Log("Failed to create descriptor pool!");
		}
	}
	...
	{	VkPrivateDataSlotCreateInfo create_info { VK_STRUCTURE_TYPE_PRIVATE_DATA_SLOT_CREATE_INFO };
		...
		{
			Debug::Log("Failed to create private data slot!");
		}
	}
}
reshade::vulkan::device_impl::~device_impl()
	...
	case api::resource_type::buffer:
		{
			...
			if (vk.CreateBuffer(_orig, &create_info, _alloc->GetAllocationCallbacks(), &object) == VK_SUCCESS)
			{
...
bool reshade::vulkan::device_impl::map_texture_region(api::resource resource, uint32_t subresource, const api::subresource_box *box, api::map_access, api::subresource_data *out_data)
{
	...
	if (resource_data->allocation != nullptr)
	{
...
void reshade::vulkan::device_impl::update_texture_region(const api::subresource_data &data, api::resource dst, uint32_t dst_subresource, const api::subresource_box *dst_box)
{
	...
	{
		Debug::Log("Failed to create upload buffer (width = %llu)!", total_image_size);
		return;
	}
