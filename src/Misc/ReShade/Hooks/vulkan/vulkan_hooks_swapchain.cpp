...
{
	Debug::Log("Redirecting vkCreateSwapchainKHR(device = %p, pCreateInfo = %p, pAllocator = %p, pSwapchain = %p) ...", device, pCreateInfo, pAllocator, pSwapchain);
	...
	// Dump swap chain description
	{
		...
		Debug::Log("> Dumping swap chain description:");
		Debug::Log("  +-----------------------------------------+-----------------------------------------+");
		Debug::Log("  | Parameter                               | Value                                   |");
		Debug::Log("  +-----------------------------------------+-----------------------------------------+");
		Debug::Log("  | flags                                   |"                               " %-#39x |", static_cast<unsigned int>(create_info.flags));
		Debug::Log("  | surface                                 |"                                " %-39p |", create_info.surface);
		Debug::Log("  | minImageCount                           |"                                " %-39u |", create_info.minImageCount);
		if (format_string != nullptr)
		Debug::Log("  | imageFormat                             |"                                " %-39s |", format_string);
		else
		Debug::Log("  | imageFormat                             |"                                " %-39d |", static_cast<int>(create_info.imageFormat));
		if (color_space_string != nullptr)
		Debug::Log("  | imageColorSpace                         |"                                " %-39s |", color_space_string);
		else
		Debug::Log("  | imageColorSpace                         |"                                " %-39d |", static_cast<int>(create_info.imageColorSpace));
		Debug::Log("  | imageExtent                             |"            " %-19u"            " %-19u |", create_info.imageExtent.width, create_info.imageExtent.height);
		Debug::Log("  | imageArrayLayers                        |"                                " %-39u |", create_info.imageArrayLayers);
		Debug::Log("  | imageUsage                              |"                               " %-#39x |", static_cast<unsigned int>(create_info.imageUsage));
		Debug::Log("  | imageSharingMode                        |"                                " %-39d |", static_cast<int>(create_info.imageSharingMode));
		Debug::Log("  | queueFamilyIndexCount                   |"                                " %-39u |", create_info.queueFamilyIndexCount);
		Debug::Log("  | preTransform                            |"                               " %-#39x |", static_cast<unsigned int>(create_info.preTransform));
		Debug::Log("  | compositeAlpha                          |"                               " %-#39x |", static_cast<unsigned int>(create_info.compositeAlpha));
		Debug::Log("  | presentMode                             |"                                " %-39d |", static_cast<int>(create_info.presentMode));
		Debug::Log("  | clipped                                 |"                                " %-39s |", create_info.clipped ? "true" : "false");
		Debug::Log("  | oldSwapchain                            |"                                " %-39p |", create_info.oldSwapchain);
		Debug::Log("  +-----------------------------------------+-----------------------------------------+");
	}
	...
	{
		Debug::Log("vkCreateSwapchainKHR failed with error code %d.", static_cast<int>(result));
		return result;
	}
	...
	Debug::Log(reshade::log::level::debug, "Returning Vulkan swap chain %p.", *pSwapchain);
#endif
	return result;
}
void     VKAPI_CALL vkDestroySwapchainKHR(VkDevice device, VkSwapchainKHR swapchain, const VkAllocationCallbacks *pAllocator)
{
	Debug::Log("Redirecting vkDestroySwapchainKHR(device = %p, swapchain = %p, pAllocator = %p) ...", device, swapchain, pAllocator);
