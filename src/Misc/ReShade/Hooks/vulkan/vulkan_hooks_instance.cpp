...
{
	Debug::Log("Redirecting vkCreateInstance(pCreateInfo = %p, pAllocator = %p, pInstance = %p) ...", pCreateInfo, pAllocator, pInstance);
	...
	Debug::Log("> Dumping enabled instance extensions:");
	...
	Debug::Log("Requesting new Vulkan instance for API version %u.%u.", VK_API_VERSION_MAJOR(app_info.apiVersion), VK_API_VERSION_MINOR(app_info.apiVersion));
	...
	{
		Debug::Log("> Replacing requested version with 1.1.");
	...
	{
		Debug::Log("> Replacing requested version with 1.3.");
...

	{
		...
		const auto add_extension = [&extensions, &enabled_extensions](const char *name, bool required) {
			...
			if (required)
			{
				Debug::Log("Required extension \"%s\" is not supported on this instance. Initialization failed.", name);
			}
			else
			{
				Debug::Log("Optional extension \"%s\" is not supported on this instance.", name);
			}
...
#endif
		Debug::Log("vkCreateInstance failed with error code %d.", static_cast<int>(result));
		return result;
	}
	...
	Debug::Log(reshade::log::level::debug, "Returning Vulkan instance %p.", instance.handle);
#endif
	return VK_SUCCESS;
}
void     VKAPI_CALL vkDestroyInstance(VkInstance instance, const VkAllocationCallbacks *pAllocator)
{
	Debug::Log("Redirecting vkDestroyInstance(instance = %p, pAllocator = %p) ...", instance, pAllocator);
...
{
	Debug::Log("Redirecting vkCreateWin32SurfaceKHR(instance = %p, pCreateInfo = %p, pAllocator = %p, pSurface = %p) ...", instance, pCreateInfo, pAllocator, pSurface);
	...
	if (result != VK_SUCCESS)
	{
		Debug::Log("vkCreateWin32SurfaceKHR failed with error code %d.", static_cast<int>(result));
		return result;
	}
...
{
	Debug::Log("Redirecting vkDestroySurfaceKHR(instance = %p, surface = %p, pAllocator = %) ...", instance, surface, pAllocator);
