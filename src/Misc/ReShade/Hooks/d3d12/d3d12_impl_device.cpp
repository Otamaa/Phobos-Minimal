	...
	_gpu_view_heap(device)
{
	...
	// Create mipmap generation states used in 'command_list::generate_mipmaps' implementation
	{
		...
		{
			Debug::Log("Failed to create mipmap generation signature!");
		}
		else
		{
			...
			{
				Debug::Log("Failed to create mipmap generation pipeline!");
			}
		}
	}
}
...
void reshade::d3d12::device_impl::update_buffer_region(const void *data, api::resource dst, uint64_t dst_offset, uint64_t size)
{
	...
	{
		Debug::Log("Failed to create upload buffer (width = %llu)!", size);
		return;
	}
...
void reshade::d3d12::device_impl::update_texture_region(const api::subresource_data &data, api::resource dst, uint32_t dst_subresource, const api::subresource_box *dst_box)
{
	...
	{
		Debug::Log("Failed to create upload buffer (width = %llu)!", internal_desc.Width);
		return;
	}
	...
	*out_layout = { 0 };
	...
	else
	{
		if (error_blob != nullptr)
			Debug::Log("Failed to create root signature: %s", static_cast<const char *>(error_blob->GetBufferPointer()));
...
void D3D12DescriptorHeap::initialize_descriptor_base_handle(size_t heap_index)
{
	...
	{
		reshade::Debug::Log(reshade::log::level::error, "Descriptor heap index is too big to fit into handle!");
	}
#else
	...
	{
		assert(false);
		reshade::Debug::Log(reshade::log::level::error, "Descriptor heap contains descriptors that are too small!");
	}
	if ((heap_desc.NumDescriptors * _device->GetDescriptorHandleIncrementSize(heap_desc.Type)) >= (1 << heap_index_start))
	{
		assert(false);
		reshade::Debug::Log(reshade::log::level::error, "Descriptor heap contains too many descriptors to fit into handle!");
	}
