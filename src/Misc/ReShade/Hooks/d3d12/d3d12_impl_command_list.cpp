...
void reshade::d3d12::command_list_impl::push_descriptors(api::shader_stage stages, api::pipeline_layout layout, uint32_t layout_param, const api::descriptor_table_update &update)
{
	...
	{
		Debug::Log("Failed to allocate %u transient descriptor handle(s) of type %u!", update.count, static_cast<uint32_t>(update.type));
		return;
	}
...
void reshade::d3d12::command_list_impl::clear_unordered_access_view_uint(api::resource_view uav, const uint32_t values[4], uint32_t rect_count, const api::rect *rects)
{
	...
	{
		Debug::Log("Failed to allocate %u transient descriptor handle(s) of type %u!", 1u, static_cast<uint32_t>(api::descriptor_type::unordered_access_view));
		return;
	}
...
void reshade::d3d12::command_list_impl::generate_mipmaps(api::resource_view srv)
{
	...
	{
		Debug::Log("Failed to allocate %u transient descriptor handle(s) of type %u!", levels_multiple_of_6, static_cast<uint32_t>(api::descriptor_type::unordered_access_view));
		return;
	}
