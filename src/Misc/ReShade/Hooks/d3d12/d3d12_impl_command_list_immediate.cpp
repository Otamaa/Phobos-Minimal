...
bool reshade::d3d12::command_list_immediate_impl::flush(bool wait)
{
	...
	{
		Debug::Log("Failed to close immediate command list with error code %s!", Debug::hr_to_string(hr).c_str());
	...
	{
		Debug::Log("Failed to reset immediate command list with error code %s!", Debug::hr_to_string(hr).c_str());
		return false;
	}
