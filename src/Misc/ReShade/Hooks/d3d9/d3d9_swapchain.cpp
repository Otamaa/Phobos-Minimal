...
ULONG   STDMETHODCALLTYPE Direct3DSwapChain9::Release()
{
	...
	if (ref_orig != 0) // Verify internal reference count
		Debug::Log("Reference count for IDirect3DSwapChain9%s object %p (%p) is inconsistent (%lu).", extended_interface ? "Ex" : "", this, orig, ref_orig);
...
void Direct3DSwapChain9::on_finish_present(HRESULT hr)
{
	...
	{
		Debug::Log("Device was lost with %s!", Debug::hr_to_string(hr).c_str());
