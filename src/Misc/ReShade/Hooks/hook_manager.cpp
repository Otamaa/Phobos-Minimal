...
static bool install_internal(const char *name, reshade::hook &hook, hook_method method)
{
	...
	{
		Debug::LogDeferred("Failed to install hook for %s with status code %d!", name, static_cast<int>(status));
		return false;
	}
...
static bool install_internal(HMODULE target_module, HMODULE replacement_module, hook_method method)
{
	...
	{
		Debug::LogDeferred("> Invalid module! Skipped.");
		return false;
	}
	...
	{
		Debug::LogDeferred( "> Empty export table! Skipped.");
		return false;
	}
	...
	Debug::LogDeferred("> Found %zu match(es). Installing ...", matches.size());
...
static bool uninstall_internal(const char *name, reshade::hook &hook, hook_method method)
{
	...
	if (hook.uninstalled())
	{
		Debug::LogDeferred( "Hook for %s was already uninstalled.", name);
		return true;
	}
	...
	if (status != reshade::hook::status::success)
	{
		Debug::LogDeferred( "Failed to uninstall hook for %s with status code %d.", name, static_cast<int>(status));
		return false;
	}
...
static void install_delayed_hooks(const std::filesystem::path &loaded_path, bool check_all = false)
{
	...
	if (!lock.owns_lock())
	{
		Debug::LogDeferred( "Ignoring LoadLibrary('%s') call to avoid possible deadlock.", loaded_path.u8string().c_str());
		return;
	}
	...
	const auto check_delayed_hook_path = [](const std::filesystem::path &path) -> HMODULE {
		...
		if (!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_PIN, path.c_str(), &delayed_handle) || delayed_handle == ReShadeIntegration::g_module_handle)
			return nullptr;
	...
	const auto install_delayed_hook_path = [&loaded_path](const std::filesystem::path &path, HMODULE delayed_handle) {
		Debug::LogDeferred("Installing delayed hooks for '%s' (Just loaded via LoadLibrary('%s')) ...", path.u8string().c_str(), loaded_path.u8string().c_str());

		install_internal(delayed_handle, ReShadeIntegration::g_module_handle, hook_method::function_hook);
		if (!reshade::hook::apply_queued_actions())
			Debug::LogDeferred("Failed to install hooks for '%s'!", path.u8string().c_str());
	};
...
HMODULE WINAPI HookLoadLibraryA(LPCSTR lpFileName)
{
	...
	if (handle != nullptr && handle != ReShadeIntegration::g_module_handle)
...
HMODULE WINAPI HookLoadLibraryExA(LPCSTR lpFileName, HANDLE hFile, DWORD dwFlags)
{
	...
	if (handle != nullptr && handle != ReShadeIntegration::g_module_handle && (dwFlags & (LOAD_LIBRARY_AS_DATAFILE | LOAD_LIBRARY_AS_DATAFILE_EXCLUSIVE | LOAD_LIBRARY_AS_IMAGE_RESOURCE)) == 0)
...
{
	Debug::Log("Uninstalling %zu hook(s) ...", s_hooks.size());
	...
	if (g_export_module_handle)
	{
		if (!FreeLibrary(g_export_module_handle))
			Debug::Log("Failed to unload '%s' with error code %lu!", s_export_module_path.u8string().c_str(), GetLastError());
...
void reshade::hooks::register_module(const std::filesystem::path &target_path)
{
	if (s_dll_notification_cookie == nullptr)
	{
			...
			LdrRegisterDllNotification(0, reinterpret_cast<FARPROC>(&DllNotificationCallback), nullptr, &s_dll_notification_cookie) != 0 /* STATUS_SUCCESS */)
		{
			...
			if (hook::apply_queued_actions())
				...
				Debug::LogDeferred("Failed to install LoadLibrary hooks!");
		}
	}

	Debug::LogDeferred("Registering hooks for '%s' ...", target_path.u8string().c_str());
	...
	const std::filesystem::path replacement_name = ReShadeIntegration::g_reshade_dll_path.filename();
	if (_wcsicmp(target_name.c_str(), replacement_name.c_str()) == 0)
	{
		assert(target_path != ReShadeIntegration::g_reshade_dll_path && target_path.is_absolute());

		if (!s_export_module_path.empty())
		{
			Debug::LogDeferred("> Skipped.");
		}
		else
		{
			Debug::LogDeferred("> Delayed until first call to an exported function.");
	...
	{
		Debug::LogDeferred("> Delayed.");

		s_delayed_hook_paths.push_back(target_path);
	}
	else // The target module is already loaded, so we can safely install hooks right away
	{
		Debug::LogDeferred("> Libraries loaded.");

		install_internal(handle, ReShadeIntegration::g_module_handle, hook_method::function_hook);
		if (!hook::apply_queued_actions())
			Debug::LogDeferred("Failed to install hooks for '%s'!", target_path.u8string().c_str());
	}
}
...
void reshade::hooks::ensure_export_module_loaded()
{
	...
	if (!g_export_module_handle && !s_export_module_path.empty())
	{
		...
		Debug::LogDeferred("Installing export hooks for '%s' ...", s_export_module_path.u8string().c_str());
		...
		if (handle != nullptr)
		{
			assert(handle != ReShadeIntegration::g_module_handle);

			install_internal(handle, ReShadeIntegration::g_module_handle, hook_method::export_hook);
		...
		{
			Debug::LogDeferred("Failed to load '%s' with error code %lu!", s_export_module_path.u8string().c_str(), GetLastError());
		}
	}
}
...
reshade::hook::address reshade::hooks::call(hook::address replacement, hook::address target)
{
	...
	Debug::LogDeferred("Unable to resolve hook for %p!", replacement);
