...
bool reshadefx::preprocessor::append_string(std::string source_code, const std::filesystem::path &path)
{
	...
	push(std::move(source_code), path.empty() ?
		"unknown" : std::string(u8Str.begin(), u8Str.end()));
...
void reshadefx::preprocessor::parse_include()
{
	...
	const std::u8string _file_path_str = file_path.u8string();
	const std::string file_path_string = std::string(_file_path_str.begin(), _file_path_str.end());
	...
	else
	{
		if (!read_file(file_path, input)){
			const std::u8string _file_name = file_name.u8string();
			return error(keyword_location, "could not open included file '" + std::string(_file_name.begin(), _file_name.end()) + '\'');
		}
...
bool reshadefx::preprocessor::evaluate_identifier_as_macro()
{
	...
	if (_token.literal_as_string == "__FILE_STEM__")
	{
		...
		const std::u8string _file_stem = file_stem.u8string();
		push(escape_string(std::string(_file_stem.begin(), _file_stem.end())));
		return true;
	}
	if (_token.literal_as_string == "__FILE_STEM_HASH__")
	{
		const std::filesystem::path file_stem = std::filesystem::u8path(_token.location.source).stem();
		const std::u8string _file_stem = file_stem.u8string();
		push(std::to_string(std::hash<std::string>()(std::string(_file_stem.begin(), _file_stem.end())) & 0xFFFFFFFF));
		return true;
	}
	if (_token.literal_as_string == "__FILE_NAME__")
	{
		...
		const std::u8string _file_name = file_name.u8string();
		push(escape_string(std::string(_file_name.begin(), _file_name.end())));
		return true;
	}
	if (_token.literal_as_string == "__FILE_NAME_HASH__")
	{
		const std::filesystem::path file_name = std::filesystem::u8path(_token.location.source).filename();
		const std::u8string _file_name = file_name.u8string();
		push(std::to_string(std::hash<std::string>()(std::string(_file_name.begin(), _file_name.end())) & 0xFFFFFFFF));
		return true;
	}
