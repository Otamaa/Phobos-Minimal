...
#include "../FX/effect_parser.hpp"
#include "../FX/effect_codegen.hpp"
#include "../FX/effect_preprocessor.hpp"
...
#include <smmintrin.h>
#include <immintrin.h>
#include <Lib/fpng/fpng.h>
#include <Lib/jxl_simple_lossless/simple_lossless.h>
#include <Lib/stb/stb_image.h>
#include <Lib/stb_image/stb_image_dds.h>
#include <Lib/stb/stb_image_write.h>
#include <Lib/stb_image/stb_image_write_hdr_png.h>
...
bool resolve_path(std::filesystem::path &path, std::error_code &ec, const std::filesystem::path &base = g_reshade_base_path)
{
	...
	path = std::filesystem::path(expand_macro_string(path.string()));
...
static bool find_file(const std::vector<std::filesystem::path> &search_paths, std::filesystem::path &path)
{
	...
	for (std::filesystem::path search_path : search_paths)
	{
		...
		{
			reshade::log::message(reshade::log::level::warning, "Failed to resolve search path '%s' with error code %d.", search_path.string().c_str(), ec.value());
		}
	}
...
bool reshade::runtime::on_init()
{
	...
	log::message(log::level::info, "Recreated runtime environment on runtime %p ('%s').", this, _config_path.string().c_str());
...
void reshade::runtime::on_reset()
{
	...
	log::message(log::level::info, "Destroyed runtime environment on runtime %p ('%s').", this, _config_path.string().c_str());
}
...
void reshade::runtime::load_config()
{
	...
	if (!resolve_path(_effect_cache_path, ec))
	{
		...
		if (ec)
			log::message(log::level::error, "Failed to create effect cache directory '%s' with error code %d!", _effect_cache_path.string().c_str(), ec.value());
	}
...
void reshade::runtime::load_current_preset()
{
	...
	for (const effect &effect : _effects)
		preset.get(effect.source_file.filename().string(), "PreprocessorDefinitions", preset_preprocessor_definitions[effect.source_file.filename().string()]);
		...
		if (std::find_if(technique_list.cbegin(), technique_list.cend(),
				[this](const std::string_view technique_name) {
					...
					const auto it = std::find_if(_effects.cbegin(), _effects.cend(),
						[effect_name = std::filesystem::path(technique_name.substr(at_pos + 1))](const effect &effect) {
	...
	for (const std::string_view technique_name : technique_list)
	{
				...
				}) == _techniques.end())
		{
			if (_reload_remaining_effects == 0)
				log::message(log::level::warning, "Preset '%s' uses unknown technique '%*s'.", _current_preset_path.string().c_str(), technique_name.size(), technique_name.data());
	...
	std::stable_sort(_technique_sorting.begin(), _technique_sorting.end(),
		[this, &sorted_technique_list](size_t lhs_technique_index, size_t rhs_technique_index) {
			...
			const std::string lhs_unique = lhs.name + '@' + _effects[lhs.effect_index].source_file.filename().string();
			...
			const std::string rhs_unique = rhs.name + '@' + _effects[rhs.effect_index].source_file.filename().string();
	...
	for (effect &effect : _effects)
	{
		const std::string effect_name = effect.source_file.filename().string();
	...
	for (technique &tech : _techniques)
	{
		const std::string unique_name = tech.name + '@' + _effects[tech.effect_index].source_file.filename().string();
...
bool reshade::runtime::load_effect(const std::filesystem::path &source_file, const ini_file &preset, size_t effect_index, size_t permutation_index, bool force_load, bool preprocess_required)
{
	...
	attributes += "app=" + g_target_executable_path.stem().string() + ';';
	...
	for (const std::filesystem::path &include_path : include_paths)
	{
		for (const std::filesystem::directory_entry &entry : std::filesystem::directory_iterator(include_path, std::filesystem::directory_options::skip_permission_denied, ec))
		{
			...
			{
				attributes += entry.path().filename().string();
				attributes += '?';
	...
	if (!preprocessed && (preprocess_required || (source_cached = load_effect_cache(source_file.stem().string() + '-' + std::to_string(_renderer_id) + '-' + std::to_string(source_hash), "i", source)) == false))
	{
		...
		if (preprocessed)
		{
			...
			source_cached = save_effect_cache(source_file.stem().string() + '-' + std::to_string(_renderer_id) + '-' + std::to_string(source_hash), "i", source);
		}
	...
	if (!compiled && !source.empty())
	{
		...
		permutation.Rmodule = codegen->Rmodule();
		...
		if (compiled)
		{
			if (permutation_index == 0)
			{
				...
				effect.uniform_data_storage.resize((permutation.Rmodule.total_uniform_size + 15) & ~15);

				for (uniform variable : permutation.Rmodule.uniforms)
				{
			...
			{
				if (permutation.Rmodule.total_uniform_size != effect.permutations[0].Rmodule.total_uniform_size ||
					!std::equal(
						permutation.Rmodule.uniforms.begin(), permutation.Rmodule.uniforms.end(),
						effect.permutations[0].Rmodule.uniforms.begin(), effect.permutations[0].Rmodule.uniforms.end(),
						...
						}))
				{
					...
					lhs_uniform_names.reserve(permutation.Rmodule.uniforms.size());
					std::transform(
						permutation.Rmodule.uniforms.begin(), permutation.Rmodule.uniforms.end(),
					...
					rhs_uniform_names.reserve(effect.permutations[0].Rmodule.uniforms.size());
					std::transform(
						effect.permutations[0].Rmodule.uniforms.begin(), effect.permutations[0].Rmodule.uniforms.end(),
				...
				if (!std::equal(
						permutation.Rmodule.techniques.begin(), permutation.Rmodule.techniques.end(),
						effect.permutations[0].Rmodule.techniques.begin(), effect.permutations[0].Rmodule.techniques.end(),
						...
						}))
				{
					...
					lhs_technique_names.reserve(permutation.Rmodule.techniques.size());
					std::transform(
						permutation.Rmodule.techniques.begin(), permutation.Rmodule.techniques.end(),
					...
					rhs_technique_names.reserve(effect.permutations[0].Rmodule.techniques.size());
					std::transform(
						effect.permutations[0].Rmodule.techniques.begin(), effect.permutations[0].Rmodule.techniques.end(),
			...
			if (_performance_mode)
			{
				...
				for (reshadefx::uniform &spec_constant : permutation.Rmodule.spec_constants)
				{
				...
				codegen->Rmodule().spec_constants = permutation.Rmodule.spec_constants;
			}
		}
	...
	if ((preprocessed || source_cached) && compiled)
	{
		if (permutation.cso.empty())
		{
			...
			for (const std::pair<std::string, reshadefx::shader_type> &entry_point : permutation.Rmodule.entry_points)
			{
				...
				const std::string cache_id = source_file.stem().string() + '-' + std::to_string(_renderer_id) + '-' + std::to_string(source_hash) + '-' + std::to_string(spec_constants_hash) + '-' + entry_point.first;
		...
		for (texture new_texture : permutation.Rmodule.textures)
		{
				...
				existing_texture != _textures.end())
			{
				...
				if (new_texture.semantic != existing_texture->semantic)
				{
					errors += "error: " + new_texture.unique_name + ":"
						" another effect " + (shared_permutation ? "permutation" : '(' + _effects[existing_texture->shared[0]].source_file.filename().string() + ')') +
				...
				{
					for (const reshadefx::sampler &sampler_info : permutation.Rmodule.samplers)
					{
			...
			if (new_texture.annotation_as_int("pooled") && new_texture.semantic.empty())
			{
					...
					existing_texture != _textures.end())
				{
					...
					for (reshadefx::sampler &sampler_info : permutation.Rmodule.samplers)
					{
					...
					for (reshadefx::storage &storage_info : permutation.Rmodule.storages)
					{
					...
					for (reshadefx::technique &tech : permutation.Rmodule.techniques)
					{
		...
		for (technique new_technique : permutation.Rmodule.techniques)
		{
	...
	if (compiled && (preprocessed || source_cached))
	{
		if (effect.errors.empty())
			log::message(log::level::info, "Successfully compiled '%s'%s in %f s.", source_file.string().c_str(), permutation_index == 0 ? "" : " permutation", std::chrono::duration_cast<std::chrono::milliseconds>(time_load_finished - time_load_started).count() * 1e-3f);
		else
			log::message(log::level::warning, "Successfully compiled '%s'%s in %f s with warnings:\n%s", source_file.string().c_str(), permutation_index == 0 ? "" : " permutation", std::chrono::duration_cast<std::chrono::milliseconds>(time_load_finished - time_load_started).count() * 1e-3f, effect.errors.c_str());
		return true;
	}
	else
	{
		...
		if (effect.errors.empty())
			log::message(log::level::error, "Failed to compile '%s'%s!", source_file.string().c_str(), permutation_index == 0 ? "" : " permutation");
		else
			log::message(log::level::error, "Failed to compile '%s'%s:\n%s", source_file.string().c_str(), permutation_index == 0 ? "" : " permutation", effect.errors.c_str());
		return false;
	}
}
bool reshade::runtime::create_effect(size_t effect_index, size_t permutation_index)
{
	...
	for (const reshadefx::uniform &spec_constant : permutation.Rmodule.spec_constants)
	{
	...
	for (const reshadefx::technique &tech : permutation.Rmodule.techniques)
	{
	...
	if (permutation_index == 0 &&
		!_device->create_query_heap(api::query_type::timestamp, static_cast<uint32_t>((permutation.Rmodule.techniques.size() + total_pass_count) * 2 * 4), &effect.query_heap))
	{
		log::message(log::level::error, "Failed to create query heap for effect file '%s'!", effect.source_file.string().c_str());
	}
	...
	// Create pipeline layout for this effect
	{
		...
		{
			log::message(log::level::error, "Failed to create pipeline layout for effect file '%s'!", effect.source_file.string().c_str());
			return false;
		}
	}
	...
	if (sampler_range.count != 0)
	{
		...
		{
			log::message(log::level::error, "Failed to create sampler descriptor table for effect file '%s'!", effect.source_file.string().c_str());
			goto exit_failure;
		}
	}
	if (srv_range.count != 0 && !sampler_with_resource_view)
	{
		...
		{
			log::message(log::level::error, "Failed to create texture descriptor table for effect file '%s'!", effect.source_file.string().c_str());
			goto exit_failure;
		}
	}
	if (uav_range.count != 0)
	{
		...
		{
			log::message(log::level::error, "Failed to create storage descriptor table for effect file '%s'!", effect.source_file.string().c_str());
			goto exit_failure;
		}
	}
		...
		for (size_t pass_index = 0; pass_index < tech.permutations[permutation_index].passes.size(); ++pass_index, ++pass_index_in_effect)
		{
			...
			if (!pass.cs_entry_point.empty())
			{
				...
				if (_renderer_id & 0x20000)
				{
					...
					cs_desc.spec_constants = static_cast<uint32_t>(permutation.Rmodule.spec_constants.size());
				...
				if (!_device->create_pipeline(permutation.layout, static_cast<uint32_t>(subobjects.size()), subobjects.data(), &pass.pipeline))
				{
					...
					log::message(log::level::error, "Failed to create compute pipeline for pass %zu in technique '%s' in '%s'!", pass_index, tech.name.c_str(), effect.source_file.string().c_str());
					goto exit_failure;
				}
			}
			else
			{
				...
				if (!pass.vs_entry_point.empty())
				{
					...
					if (_renderer_id & 0x20000)
					{
						...
						vs_desc.spec_constants = static_cast<uint32_t>(permutation.Rmodule.spec_constants.size());
				...
				if (!pass.ps_entry_point.empty())
				{
					...
					if (_renderer_id & 0x20000)
					{
						...
						ps_desc.spec_constants = static_cast<uint32_t>(permutation.Rmodule.spec_constants.size());
				...
				if (!_device->create_pipeline(permutation.layout, static_cast<uint32_t>(subobjects.size()), subobjects.data(), &pass.pipeline))
				{
					...
					log::message(log::level::error, "Failed to create graphics pipeline for pass %zu in technique '%s' in '%s'!", pass_index, tech.name.c_str(), effect.source_file.string().c_str());
					goto exit_failure;
				}
			}
			...
			for (const reshadefx::sampler_binding &binding : pass.sampler_bindings)
			{
				...
				const reshadefx::sampler &sampler_info = permutation.Rmodule.samplers[binding.index];
				...
				{
					log::message(log::level::error, "Failed to create sampler object in '%s'!", effect.source_file.string().c_str());
					goto exit_failure;
				}
			...
			for (const reshadefx::texture_binding &binding : pass.texture_bindings)
			{
				const auto sampler_texture = std::find_if(_textures.cbegin(), _textures.cend(),
					[&unique_name = permutation.Rmodule.samplers[binding.index].texture_name](const texture &item) {
						return item.unique_name == unique_name && (item.resource != 0 || !item.semantic.empty());
					});
			...
			for (const reshadefx::storage_binding &binding : pass.storage_bindings)
			{
				const auto storage_texture = std::find_if(_textures.cbegin(), _textures.cend(),
					[&unique_name = permutation.Rmodule.storages[binding.index].texture_name](const texture &item) {
						return item.unique_name == unique_name && (item.resource != 0 || !item.semantic.empty());
					});
				assert(storage_texture != _textures.cend());

				const api::resource_view &uav = storage_texture->uav[permutation.Rmodule.storages[binding.index].level];
...
void reshade::runtime::load_textures(size_t effect_index)
{
	for (texture &tex : _textures)
	{
		...
		std::filesystem::path source_path = std::filesystem::path(tex.annotation_as_string("source"));
		...
		{
			log::message(log::level::error, "Source '%s' for texture '%s' was not found in any of the texture search paths!", source_path.string().c_str(), tex.unique_name.c_str());
			_last_reload_successful = false;
			continue;
		}
		...
		if (FILE *const file = _wfsopen(source_path.c_str(), L"rb", SH_DENYNO))
		{
			...
			if (source_path.extension() == L".cube")
			{
				...
				{
					log::message(log::level::error, "Source '%s' for texture '%s' is a Cube LUT file, which can only be loaded into textures with a floating-point format!", source_path.string().c_str(), tex.unique_name.c_str());
					_last_reload_successful = false;
					continue;
				}
		...
		if (pixels == nullptr)
		{
			log::message(log::level::error, "Failed to load '%s' for texture '%s'!", source_path.string().c_str(), tex.unique_name.c_str());
			_last_reload_successful = false;
			continue;
		}
...
void reshade::runtime::load_effects(bool force_load_all)
{
	...
	for (const std::filesystem::path &effect_file : effect_files)
		preset.get(effect_file.filename().string(), "PreprocessorDefinitions", _preset_preprocessor_definitions[effect_file.filename().string()]);
...
bool reshade::runtime::load_effect_cache(const std::string &id, const std::string &type, std::string &data) const
{
	...
	path /= std::filesystem::path("reshade-" + id + '.' + type);
	...
	for (size_t pass_index = 0; pass_index < tech.permutations[permutation_index].passes.size(); ++pass_index)
	{
		...
		else
		{
			...
			if (_device->get_api() == api::device_api::d3d9)
			{
				...
				for (const reshadefx::texture &tex : permutation.Rmodule.textures)
				{
					if (tex.semantic.empty())
						continue;
...
void reshade::runtime::save_texture(const texture &tex)
{
	...
	const std::filesystem::path screenshot_path = g_reshade_base_path / _screenshot_path / std::filesystem::path(screenshot_name);
...
void reshade::runtime::save_screenshot(const char *postfix_in)
{
	...
	std::string screenshot_name = expand_macro_string(_screenshot_name, {
		{ "AppName", g_target_executable_path.stem().string() },
		{ "PresetName", _current_preset_path.stem().string() },
		{ "BeforeAfter", postfix },
	...
	const std::filesystem::path screenshot_path = g_reshade_base_path / _screenshot_path / std::filesystem::path(screenshot_name).lexically_normal();

	log::message(log::level::info, "Saving screenshot to '%s'.", screenshot_path.string().c_str());
		...
		_worker_threads.emplace_back([this, screenshot_count, screenshot_format, screenshot_path, postfix, pixels = std::move(pixels), include_preset]() mutable {
			...
			if (!std::filesystem::exists(screenshot_path.parent_path(), ec))
				if (!(_screenshot_directory_creation_successful = std::filesystem::create_directories(screenshot_path.parent_path(), ec)))
					log::message(log::level::error, "Failed to create screenshot directory '%s' with error code %d!", screenshot_path.parent_path().string().c_str(), ec.value());
			...
			if (save_success)
			{
				...
				if (include_preset)
				{
					...
					if (!std::filesystem::copy_file(_current_preset_path, screenshot_preset_path, std::filesystem::copy_options::overwrite_existing, ec))
						log::message(log::level::error, "Failed to copy preset file for screenshot to '%s' with error code %d!", screenshot_preset_path.string().c_str(), ec.value());
				}
			}
			else
			{
				log::message(log::level::error, "Failed to write screenshot to '%s'!", screenshot_path.string().c_str());
			}
...
bool reshade::runtime::execute_screenshot_post_save_command(const std::filesystem::path &screenshot_path, unsigned int screenshot_count, std::string_view postfix)
{
	...
	command_line += '\"';
	command_line += _screenshot_post_save_command.string();
	command_line += '\"';
	...
	if (!_screenshot_post_save_command_arguments.empty())
	{
		...
		command_line += expand_macro_string(_screenshot_post_save_command_arguments, {
			...
			{ "TargetPath", screenshot_path.string() },
			{ "TargetDir", screenshot_path.parent_path().string() },
			{ "TargetFileName", screenshot_path.filename().string() },
			{ "TargetExt", screenshot_path.extension().string() },
			{ "TargetName", screenshot_path.stem().string() },
			{ "Count", std::to_string(screenshot_count) }
		}, _current_time);
	}
