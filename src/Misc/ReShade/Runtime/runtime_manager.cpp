...
void reshade::create_effect_runtime(api::swapchain *swapchain, api::command_queue *graphics_queue, bool vr)
{
	...
	const ini_file &config = ini_file::load_cache(g_reshade_base_path / std::filesystem::path(config_name + ".ini"));
...
void reshade::destroy_effect_runtime(api::swapchain *swapchain)
{
	...
	{
		const std::string config_name = runtime->get_config_path().stem().string();
