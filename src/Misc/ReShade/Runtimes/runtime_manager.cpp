...
void reshade::create_effect_runtime(api::swapchain *swapchain, api::command_queue *graphics_queue, bool vr)
{
	...
	const ini_file &config = ini_file::load_cache(ReShadeIntegration::g_reshade_base_path / std::filesystem::u8path(config_name + ".ini"));
...
void reshade::destroy_effect_runtime(api::swapchain *swapchain)
{
	...
	{
		const auto _configu8 = runtime->get_config_path().stem().u8string();
		const std::string config_name = std::string(_configu8.begin(), _configu8.end());
