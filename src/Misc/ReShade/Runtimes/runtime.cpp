...
static bool find_file(const std::vector<std::filesystem::path> &search_paths, std::filesystem::path &path)
{
	...
	for (std::filesystem::path search_path : search_paths)
	{
		...
		{
			Debug::Log("Failed to resolve search path '%s' with error code %d.", search_path.u8string().c_str(), ec.value());
		}
	}
	...
	_screenshot_post_save_command_working_directory(L".\\")
{
	...
	if (uint32_t driver_version = 0;
		...
		Debug::Log(log::level::info, "Running on %s Driver %u.%u.", device_description, driver_version / 100, driver_version % 100);
	else
		Debug::Log(log::level::info, "Running on %s.", device_description);
...
bool reshade::runtime::on_init()
{
		...
		(_back_buffer_format == api::format::r8g8b8x8_unorm || _back_buffer_format == api::format::b8g8r8x8_unorm))
	{
		...
		if (!_device->create_resource(
			...
			!_device->create_resource_view(
				...
				&_back_buffer_targets.emplace_back()))
		{
			Debug::Log(, "Failed to create resolve texture resource!");
			goto exit_failure;
		}
		...
		if (need_copy_pipeline)
		{
			...
			{
				Debug::Log(, "Failed to create resolve shader resource view!");
				goto exit_failure;
			}
			...
			{
				Debug::Log(, "Failed to create copy pipeline!");
				goto exit_failure;
			}
		}
	}
	...
	if (_empty_tex == 0)
	{
		...
		{
			Debug::Log(, "Failed to create empty texture resource!");
			goto exit_failure;
		}
		...
		{
			Debug::Log(, "Failed to create empty texture shader resource view!");
			goto exit_failure;
		}
	}
	...
	for (uint32_t i = 0, count = _swapchain->get_back_buffer_count(); i < count; ++i)
	{
		...
		if (!_device->create_resource_view(
			...
			!_device->create_resource_view(
				...
				&_back_buffer_targets.emplace_back()))
		{
			Debug::Log(, "Failed to create back buffer render targets!");
			goto exit_failure;
		}
	}
	...
	Debug::Log(log::level::info, "Recreated runtime environment on runtime %p ('%s').", this, _config_path.u8string().c_str());
...
void reshade::runtime::on_reset()
{
	...
	Debug::Log(log::level::info, "Destroyed runtime environment on runtime %p ('%s').", this, _config_path.u8string().c_str());
}
...
void reshade::runtime::load_config()
{
	...
	if (!resolve_path(_effect_cache_path, ec))
	{
		...
		if (ec)
			Debug::Log(, "Failed to create effect cache directory '%s' with error code %d!", _effect_cache_path.u8string().c_str(), ec.value());
	}
	...
	for (const std::string_view technique_name : technique_list)
	{
				...
				}) == _techniques.end())
		{
			if (_reload_remaining_effects == 0)
				Debug::Log(log::level::warning, "Preset '%s' uses unknown technique '%*s'.", _current_preset_path.u8string().c_str(), technique_name.size(), technique_name.data());
...
bool reshade::runtime::load_effect(const std::filesystem::path &source_file, const ini_file &preset, size_t effect_index, size_t permutation_index, bool force_load, bool preprocess_required)
{
	...
	if (compiled && (preprocessed || source_cached))
	{
		if (effect.errors.empty())
			Debug::Log(log::level::info, "Successfully compiled '%s'%s in %f s.", source_file.u8string().c_str(), permutation_index == 0 ? "" : " permutation", std::chrono::duration_cast<std::chrono::milliseconds>(time_load_finished - time_load_started).count() * 1e-3f);
		else
			Debug::Log(log::level::warning, "Successfully compiled '%s'%s in %f s with warnings:\n%s", source_file.u8string().c_str(), permutation_index == 0 ? "" : " permutation", std::chrono::duration_cast<std::chrono::milliseconds>(time_load_finished - time_load_started).count() * 1e-3f, effect.errors.c_str());
		return true;
	}
	else
	{
		...
		if (effect.errors.empty())
			Debug::Log(, "Failed to compile '%s'%s!", source_file.u8string().c_str(), permutation_index == 0 ? "" : " permutation");
		else
			Debug::Log(, "Failed to compile '%s'%s:\n%s", source_file.u8string().c_str(), permutation_index == 0 ? "" : " permutation", effect.errors.c_str());
		return false;
	}
}
bool reshade::runtime::create_effect(size_t effect_index, size_t permutation_index)
{
	...
	{
		Debug::Log(, "Failed to create query heap for effect file '%s'!", effect.source_file.u8string().c_str());
	}
	...
	// Create pipeline layout for this effect
	{
		...
		{
			Debug::Log(, "Failed to create pipeline layout for effect file '%s'!", effect.source_file.u8string().c_str());
			return false;
		}
	}
	...
	if (_device->get_api() != api::device_api::d3d9 && !effect.uniform_data_storage.empty())
	{
		if (permutation_index == 0)
		{
			...
			{
				Debug::Log(, "Failed to create constant buffer for effect file '%s'!", effect.source_file.u8string().c_str());
				goto exit_failure;
			}
		...
		{
			Debug::Log(, "Failed to create constant buffer descriptor table for effect file '%s'!", effect.source_file.u8string().c_str());
			goto exit_failure;
		}
	...
	if (sampler_range.count != 0)
	{
		...
		{
			Debug::Log(, "Failed to create sampler descriptor table for effect file '%s'!", effect.source_file.u8string().c_str());
			goto exit_failure;
		}
	}
	if (srv_range.count != 0 && !sampler_with_resource_view)
	{
		...
		{
			Debug::Log(, "Failed to create texture descriptor table for effect file '%s'!", effect.source_file.u8string().c_str());
			goto exit_failure;
		}
	}
	if (uav_range.count != 0)
	{
		...
		{
			Debug::Log(, "Failed to create storage descriptor table for effect file '%s'!", effect.source_file.u8string().c_str());
			goto exit_failure;
		}
	}
		...
		for (size_t pass_index = 0; pass_index < tech.permutations[permutation_index].passes.size(); ++pass_index, ++pass_index_in_effect)
		{
			...
			if (!pass.cs_entry_point.empty())
			{
				...
				if (!_device->create_pipeline(permutation.layout, static_cast<uint32_t>(subobjects.size()), subobjects.data(), &pass.pipeline))
				{
					...
					Debug::Log(, "Failed to create compute pipeline for pass %zu in technique '%s' in '%s'!", pass_index, tech.name.c_str(), effect.source_file.u8string().c_str());
					goto exit_failure;
				}
			}
			else
			{
				...
				if (!_device->create_pipeline(permutation.layout, static_cast<uint32_t>(subobjects.size()), subobjects.data(), &pass.pipeline))
				{
					...
					Debug::Log(, "Failed to create graphics pipeline for pass %zu in technique '%s' in '%s'!", pass_index, tech.name.c_str(), effect.source_file.u8string().c_str());
					goto exit_failure;
				}
			}
			...
			for (const reshadefx::sampler_binding &binding : pass.sampler_bindings)
			{
				...
				{
					Debug::Log(, "Failed to create sampler object in '%s'!", effect.source_file.u8string().c_str());
					goto exit_failure;
				}
...
void reshade::runtime::load_textures(size_t effect_index)
{
	for (texture &tex : _textures)
	{
		...
		{
			Debug::Log(, "Source '%s' for texture '%s' was not found in any of the texture search paths!", source_path.u8string().c_str(), tex.unique_name.c_str());
			_last_reload_successful = false;
			continue;
		}
		...
		if (FILE *const file = _wfsopen(source_path.c_str(), L"rb", SH_DENYNO))
		{
			...
			if (source_path.extension() == L".cube")
			{
				...
				{
					Debug::Log(, "Source '%s' for texture '%s' is a Cube LUT file, which can only be loaded into textures with a floating-point format!", source_path.u8string().c_str(), tex.unique_name.c_str());
					_last_reload_successful = false;
					continue;
				}
		...
		if (pixels == nullptr)
		{
			Debug::Log(, "Failed to load '%s' for texture '%s'!", source_path.u8string().c_str(), tex.unique_name.c_str());
			_last_reload_successful = false;
			continue;
		}
		...
		case reshadefx::texture_format::rgba32f:
			break;
		default:
			Debug::Log(, "Texture upload is not supported for format %d of texture '%s'!", static_cast<int>(tex.format), tex.unique_name.c_str());
			_last_reload_successful = false;
...
bool reshade::runtime::create_texture(texture &tex)
{
	...
	{
		Debug::Log(, "Failed to create texture '%s' (width = %u, height = %u, levels = %hu, format = %u, usage = %#x)! Make sure the texture dimensions are reasonable.", tex.unique_name.c_str(), tex.width, tex.height, tex.levels, static_cast<uint32_t>(format), static_cast<uint32_t>(usage));
		return false;
	}
	...
	// Always create shader resource views
	{
		...
		{
			Debug::Log(, "Failed to create shader resource view for texture '%s' (format = %u, levels = %hu)!", tex.unique_name.c_str(), static_cast<uint32_t>(view_format), tex.levels);
			return false;
		}
		...
		{
			Debug::Log(, "Failed to create shader resource view for texture '%s' (format = %u, levels = %hu)!", tex.unique_name.c_str(), static_cast<uint32_t>(view_format_srgb), tex.levels);
			return false;
		}
	}
	...
	if (tex.render_target)
	{
		...
		{
			Debug::Log(, "Failed to create render target view for texture '%s' (format = %u)!", tex.unique_name.c_str(), static_cast<uint32_t>(view_format));
			return false;
		}
		if (view_format_srgb == view_format || tex.storage_access) // sRGB formats do not support storage usage
		{
		...
		{
			Debug::Log(, "Failed to create render target view for texture '%s' (format = %u)!", tex.unique_name.c_str(), static_cast<uint32_t>(view_format_srgb));
			return false;
		}
	...
	if (tex.storage_access && _renderer_id >= 0xb000)
	{
		...
		for (uint16_t level = 0; level < tex.levels; ++level)
		{
			...
			{
				Debug::Log(, "Failed to create unordered access view for texture '%s' (format = %u, level = %hu)!", tex.unique_name.c_str(), static_cast<uint32_t>(view_format), level);
				return false;
			}
		}
	}
...
void reshade::runtime::clear_effect_cache()
{
	...
	if (ec)
		Debug::Log(, "Failed to clear effect cache directory with error code %d!", ec.value());
}
...
auto reshade::runtime::add_effect_permutation(uint32_t width, uint32_t height, api::format color_format, api::format stencil_format, api::color_space color_space) -> size_t
{
	...
	{
		Debug::Log(, "Failed to create effect color resource (width = %u, height = %u, format = %u)!", width, height, static_cast<uint32_t>(api::format_to_typeless(color_format)));
		goto exit_failure;
	}
	...
	{
		Debug::Log(, "Failed to create effect color resource view (format = %u)!", static_cast<uint32_t>(color_format));
		goto exit_failure;
	}
			...
			nullptr, api::resource_usage::depth_stencil_write, &permutation.stencil_tex))
	{
		...
		{
			Debug::Log(, "Failed to create effect stencil resource view (format = %u)!", static_cast<uint32_t>(stencil_format));
			goto exit_failure;
		}
	}
	else
	{
		Debug::Log(, "Failed to create effect stencil resource (width = %u, height = %u, format = %u)!", width, height, static_cast<uint32_t>(stencil_format));
...
void reshade::runtime::save_texture(const texture &tex)
{
	...
	{
		Debug::Log(, "Texture saving is not supported for 3D textures!");
		return;
	}
...
void reshade::runtime::update_texture(texture &tex, uint32_t width, uint32_t height, uint32_t depth, const void *pixels)
{
	...
	{
		Debug::Log(, "Resizing image data is not supported for 3D textures like '%s'.", tex.unique_name.c_str());
		return;
	}
	...
	{
		Debug::Log(log::level::info, "Resizing image data for texture '%s' from %ux%u to %ux%u.", tex.unique_name.c_str(), width, height, tex.width, tex.height);
...
void reshade::runtime::save_screenshot(const char *postfix_in)
{
	...
	Debug::Log(log::level::info, "Saving screenshot to '%s'.", screenshot_path.u8string().c_str());
		...
		_worker_threads.emplace_back([this, screenshot_count, screenshot_format, screenshot_path, postfix, pixels = std::move(pixels), include_preset]() mutable {
			...
			if (!std::filesystem::exists(screenshot_path.parent_path(), ec))
				if (!(_screenshot_directory_creation_successful = std::filesystem::create_directories(screenshot_path.parent_path(), ec)))
					Debug::Log(, "Failed to create screenshot directory '%s' with error code %d!", screenshot_path.parent_path().u8string().c_str(), ec.value());
			...
			if (save_success)
			{
				...
				if (include_preset)
				{
					...
					if (!std::filesystem::copy_file(_current_preset_path, screenshot_preset_path, std::filesystem::copy_options::overwrite_existing, ec))
						Debug::Log(, "Failed to copy preset file for screenshot to '%s' with error code %d!", screenshot_preset_path.u8string().c_str(), ec.value());
				}

			}
			else
			{
				Debug::Log(, "Failed to write screenshot to '%s'!", screenshot_path.u8string().c_str());
			}
...
bool reshade::runtime::execute_screenshot_post_save_command(const std::filesystem::path &screenshot_path, unsigned int screenshot_count, std::string_view postfix)
{
	...
	{
		Debug::Log(, "Failed to execute screenshot post-save command!");
		return false;
	}
...
bool reshade::runtime::get_texture_data(api::resource resource, api::resource_usage state, uint8_t *pixels, api::format quantization_format)
{
	...
	{
		Debug::Log(, "Failed to create system memory texture for screenshot capture!");
		return false;
	}
	...
	if (_device->map_texture_region(intermediate, 0, nullptr, api::map_access::read_only, &mapped_data))
	{
		...
		for (size_t y = 0; y < desc.texture.height; ++y, pixels += pixels_row_pitch, mapped_pixels += mapped_data.row_pitch)
		{
			...
			Debug::Log(, "Screenshots are not supported for format %u!", static_cast<uint32_t>(desc.texture.format));
			break;
		}
