...
std::shared_ptr<reshade::input> reshade::input::register_window(window_handle window)
{
	...
	{
		Debug::Log("Cannot capture input for window %p created by a different process (%lu).", window, process_id);
		return nullptr;
	}
...
extern "C" BOOL WINAPI HookRegisterRawInputDevices(PCRAWINPUTDEVICE pRawInputDevices, UINT uiNumDevices, UINT cbSize)
{
	...
	{
		Debug::Log("RegisterRawInputDevices failed with error code %lu.", GetLastError());
		return FALSE;
	}
