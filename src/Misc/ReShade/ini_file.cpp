...
{
	return ini_file::load_cache(ReShadeIntegration::g_reshade_base_path / L"ReShade.ini");
}
...
bool reshade::ini_file::load()
{
	...
	while (fgets(line_data.data(), static_cast<int>(line_data.size() + 1), file))
	{
		...
		const std::string_view line = PhobosCRT::trim(std::string_view(line_data.data(), line_length), " \t\r\n");
		...
		if (line[0] == '[')
		{
			section = PhobosCRT::trim(line.substr(0, line.find(']')), " \t[]");
			continue;
		}
		...
		{
			const std::string_view key = PhobosCRT::trim(line.substr(0, assign_index), " \t");
			const std::string_view value = PhobosCRT::trim(line.substr(assign_index + 1), " \t");
