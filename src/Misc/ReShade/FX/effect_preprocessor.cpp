...
bool reshadefx::preprocessor::append_string(std::string source_code, const std::filesystem::path &path)
{
	...
	push(std::move(source_code), path.empty() ? "unknown" : path.string());
	parse();
...
std::vector<std::filesystem::path> reshadefx::preprocessor::included_files() const
{
	...
	for (const std::pair<std::string, std::string> &cache_entry : _file_cache)
		files.push_back(std::filesystem::path(cache_entry.first));
	return files;
}
...
void reshadefx::preprocessor::parse_include()
{
	...
	std::filesystem::path file_name = std::filesystem::path(_token.literal_as_string);
	std::filesystem::path file_path = std::filesystem::path(_output_location.source);
	...
	else
	{
		if (!read_file(file_path, input))
			return error(keyword_location, "could not open included file '" + file_name.string() + '\'');
...
bool reshadefx::preprocessor::evaluate_identifier_as_macro()
{
	...
	{
		const std::filesystem::path file_stem = std::filesystem::path(_token.location.source).stem();
		push(escape_string(file_stem.string()));
		return true;
	}
	if (_token.literal_as_string == "__FILE_STEM_HASH__")
	{
		const std::filesystem::path file_stem = std::filesystem::path(_token.location.source).stem();
		push(std::to_string(std::hash<std::string>()(file_stem.string()) & 0xFFFFFFFF));
		return true;
	}
	if (_token.literal_as_string == "__FILE_NAME__")
	{
		const std::filesystem::path file_name = std::filesystem::path(_token.location.source).filename();
		push(escape_string(file_name.string()));
		return true;
	}
	if (_token.literal_as_string == "__FILE_NAME_HASH__")
	{
		const std::filesystem::path file_name = std::filesystem::path(_token.location.source).filename();
		push(std::to_string(std::hash<std::string>()(file_name.string()) & 0xFFFFFFFF));
		return true;
	}
