...
inline void register_destruction_callback_d3dx(OBJECT_TYPE *object, LAMBDA_TYPE &&callback)
{
	...
	com_ptr<ID3DDestructionNotifier> _notifier = nullptr;
	if (SUCCEEDED(
		object->QueryInterface(
			__uuidof(ID3DDestructionNotifier), reinterpret_cast<void**>(&_notifier)))
		)
	{
		_notifier->RegisterDestructionCallback(
			[](void *data) {
