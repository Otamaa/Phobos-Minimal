...
namespace
{
	...
	void AppendRegisters(CONTEXT* pContext)
	{
		...
		if (pContext->ContextFlags & CONTEXT_FLOATING_POINT)
		{
			...
			ExceptionHandler::Append("      Cr0NpxState: %08x\r\n", pContext->FloatSave.Spare0);
	...
	void AppendHeader()
	{
		...
		ExceptionHandler::Append("Ares version: 21.352.1218 With Unofficial Phobos version: " PRODUCT_VERSION "\r\n");
		...
		for (auto const& data : Patch::ModuleDatas) {
			ExceptionHandler::Append("Module [(%d) %s: Base address = %x]\r\n", i++, data.ModuleName.c_str(), data.BaseAddr);
		}
...
bool ExceptionHandler::WriteMinidump(EXCEPTION_POINTERS* pExs, DWORD crashedTid, bool fullMemory, char* pPathOut, size_t pathOutSize)
{
	...
	_snprintf_s(pPathOut, pathOutSize, _TRUNCATE, "%s\\%s_%02u-%02u-%04u_%02u-%02u-%02u.DMP",
		BasePath, fullMemory ? "FULLDUMP" : "CRASHDUMP",
...
void ExceptionHandler::WriteCrashArtifacts(EXCEPTION_POINTERS* pExs, unsigned int code, DWORD crashedTid)
{
	...
	_snprintf_s(ReportPath, _TRUNCATE, "%s\\EXCEPT_%02u-%02u-%04u_%02u-%02u-%02u.TXT",
		BasePath, CrashTime.wDay, CrashTime.wMonth, CrashTime.wYear,
		CrashTime.wHour, CrashTime.wMinute, CrashTime.wSecond);
