...
char ExceptionHandler::BasePath[MAX_PATH] = { '\0' };
...
void ExceptionHandler::EnsureDebugBasePath()
{
	if (BasePath[0] == '\0') {
		_snprintf_s(BasePath, _TRUNCATE, "%s\\Snapshot-%04u%02u%02u-%02u%02u%02u",
	    DebugDirectory, CrashTime.wYear, CrashTime.wDay, CrashTime.wMonth,
		CrashTime.wHour, CrashTime.wMinute, CrashTime.wSecond);
	}

	CreateDirectoryA(BasePath, nullptr);
}
