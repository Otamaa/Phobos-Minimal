...
PhobosAttachEffectTypeClass* PrismRelay::GetRelayType(TechnoClass* pTechno)
{
	if (!pTechno)
		return nullptr;
...
PhobosAttachEffectTypeClass* PrismRelay::GetRelayTypeForNetwork(TechnoClass* pTechno, int networkId)
{
	if (!pTechno)
		return nullptr;
...
int PrismRelay::GetRelayLockoutFrames(PhobosAttachEffectTypeClass* pType)
{
	if (!pType)
...
static PhobosAttachEffectTypeClass* ResolveRelayTypeForCooldown(TechnoClass* pTechno, int networkId)
{
	if (!pTechno)
		return nullptr;
...
static void StartPassCooldown(TechnoClass* pTechno, int networkId)
{
	...
	TechnoExtContainer::Instance.Find(pTechno)->PrismRelayCooldown.Start(cooldownFrames);
}

bool PrismRelay::IsOnCooldown(TechnoClass* pTechno)
{
	auto const pExt = TechnoExtContainer::Instance.Find(pTechno);
	return pExt->PrismRelayCooldown.InProgress();
}
...
static bool HasActiveRelaySession(TechnoClass* pTechno)
{
	...
	return pExt->PrismRelay.Phase != PrismRelayPhase::Idle;
}
...
static bool HasEnemyCombatTarget(TechnoClass* pTechno)
{
	...
	return cast_to<CellClass*>(pTarget) != nullptr;
}
...
static bool IsTechnoEngagedInOwnAttack(TechnoClass* pTechno)
{
	...
	{
		auto const pBuilding = cast_to<BuildingClass*, false>(pTechno);
		...
		if (BuildingExtContainer::Instance.Find(pBuilding)->IsFiringNow)
			return true;
	...
	{
		auto const pFoot = flag_cast_to<FootClass*, false>(pTechno);

		if (pFoot && pFoot->IsFiring)
			return true;

		if (auto const pUnit = cast_to<UnitClass*, false>(pTechno))
		{
	...
	{
		auto const pAircraft = cast_to<AircraftClass*, false>(pTechno);
...
bool PrismRelay::CanProvide(TechnoClass* pTechno, PhobosAttachEffectTypeClass* pType)
{
...
bool PrismRelay::CanReceive(TechnoClass* pTechno, PhobosAttachEffectTypeClass* pType)
{
...
bool PrismRelay::IsWeaponRelayAllowed(PhobosAttachEffectTypeClass* pType, WeaponTypeClass* pWeapon)
{
	if (!pType || !pWeapon)
		return false;
...
int PrismRelay::ResolveMasterFireWeaponIndex(TechnoClass* pMaster, PhobosAttachEffectTypeClass* pType, AbstractClass* pTarget, int initiatingWeaponIndex)
{
	if (!pMaster || !pType)
	...
	{
		auto const pMasterExt = TechnoExtContainer::Instance.Find(pMaster);
...
static void CompleteSupportPhase(TechnoExtData* pMasterExt);
static void OnSupportBulletEnded(TechnoExtData* pMasterExt);
static void OnSupportLayerWaveComplete(TechnoExtData* pMasterExt);
static void BeginSupportLayerFire(TechnoExtData* pMasterExt, int layer);
static void ScheduleSupportLayerFire(TechnoExtData* pMasterExt, int layer);
static void AbortSupportPhase(TechnoExtData* pMasterExt);
static void ProcessSupportFireSchedule(TechnoExtData* pMasterExt);
...
static bool IsRelayCandidateEligible(TechnoClass* pProvider, TechnoClass* pHub, TechnoClass* pMaster,
	PhobosAttachEffectTypeClass* pType, WeaponTypeClass* pSupportWeapon, const std::vector<TechnoClass*>& marked)
{
	...
	const int searchRange = WeaponTypeExtData::GetRangeWithModifiers(pSupportWeapon, pProvider);
	...
	return pProvider->InRange(pHub->Location, pHub, pSupportWeapon);
	//original , is not included in the PR for some reason
	//return WeaponTypeExtData::IsTargetInWeaponRange(pProvider, pHub, pSupportWeapon);
}

static std::vector<TechnoClass*> FindLinkCandidates(TechnoClass* pHub, TechnoClass* pMaster, PhobosAttachEffectTypeClass* pType,
	WeaponTypeClass* pSupportWeapon, const std::vector<TechnoClass*>& marked, int maxCount)
{
	...
	const int searchRange = WeaponTypeExtData::GetRangeWithModifiers(pSupportWeapon, pHub);
	...
	else
	{
		...
		auto const& technos = Helpers::Alex::getCellSpreadItems(pHub->Location, cellSpread, true ,false ,true ,true, false);
...
static RelayNetworkResult BuildNetwork(TechnoClass* pMaster, PhobosAttachEffectTypeClass* pType)
{
...
static void FireSupportLayer(TechnoExtData* pMasterExt, int layer)
{
...
static void FireSupportWave(TechnoExtData* pMasterExt)
{
	if (!pMasterExt)
		return;
...
static void ClearRelayNetworkUnitState(const std::vector<TechnoClass*>& nodes, TechnoClass* pMaster)
{
	for (auto const pTechno : nodes)
	{
		...
		auto const pTarget = flag_cast_to<TechnoClass*>(pTechno->Target);
...
static void CompleteSupportPhase(TechnoExtData* pMasterExt)
{
	...
	auto* const pMaster = pMasterExt->This();
	auto& sess = pMasterExt->PrismRelay;
...
void PrismRelay::NotifyBulletDestroyed(BulletClass* pBullet)
{
	...
	if (!pBulletExt->PrismRelaySupportBullet || pBulletExt->PrismRelayCounted)
		return;
	...
	{
		if (auto const pMasterExt = TechnoExtContainer::Instance.Find(pMaster))
