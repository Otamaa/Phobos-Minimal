...
static void FillRectTranslucent(DSurface* pSurface, RectangleStruct& rect, const ColorStruct& color, int opacity)
{
	...
	RectangleStruct bound = rect.IntersectWith(pSurface->Get_Rect());
...
bool DropshipLoadoutClass::Initialize(bool bIgnoreFixedUnits, bool bPreloadCargo, int allowableUnitsIndex, int startingMoney, Nullable<bool> bAddUnusedMoneyToPlayer, Nullable<bool> bRememberPurchasedCargo, SuperWeaponTypeClass* pSWType)
{
	...
	pHouseTypeExt = HouseTypeExtContainer::Instance.Find(HouseClass::CurrentPlayer->Type);
	...
	{
		nStartingDropships = pHouseTypeExt->DropshipLoadout_StartingDropships.isset() ? pHouseTypeExt->DropshipLoadout_StartingDropships.Get() : ScenarioExtData::Instance()->DropshipLoadout_StartingDropships;
	}
...
void DropshipLoadoutClass::LoadAssets()
{
	...
	auto pHouseExt = HouseExtContainer::Instance.Find(HouseClass::CurrentPlayer);
...
void DropshipLoadoutClass::CalculateLayout(DSurface* pSurface)
{
	...
	int backgroundX = (pSurface->Get_Width() - backgroundWidth) / 2;
	int backgroundY = (pSurface->Get_Height() - backgroundHeight) / 2;
	...
	if (pSWTypeExt)
	{
		...
		if (pSWTypeExt->DropshipLoadout_DGreenAnimationsCount.isset())
		{
			for (int i = 0; i < pSWTypeExt->DropshipLoadout_DGreenAnimationsCount; i++)
			{
				...
				dGreenLocation.emplace_back(backgroundX + location.X, backgroundY + location.Y, 0, 0);
			}
		}
		else
		{
			...
			for (int i = 0; i < 4; i++)
			{
				dGreenLocation.emplace_back(backgroundX + dGreenX, backgroundY + dGreenY, 0, 0);
				dGreenY += 50;
			}
...
void DropshipLoadoutClass::CreateControls()
{
	...
	else
	{
		...
		if (!bIgnoreFixedUnits)
		{
			if (!pHouseTypeExt->DropshipLoadout_FixedUnits.empty())
				pFixedUnitsSrc = &pHouseTypeExt->DropshipLoadout_FixedUnits;
			else if (ScenarioExtData::Instance() && !ScenarioExtData::Instance()->DropshipLoadout_FixedUnits.empty())
				pFixedUnitsSrc = &ScenarioExtData::Instance()->DropshipLoadout_FixedUnits;
		}
		...
		if (pHouseExt->DropshipLoadout_InitialUnits.empty() && !bIgnoreFixedUnits)
		{
			...
			else if (ScenarioExtData::Instance() && !ScenarioExtData::Instance()->DropshipLoadout_InitialUnits.empty())
				pHouseExt->DropshipLoadout_InitialUnits = ScenarioExtData::Instance()->DropshipLoadout_InitialUnits;
		}
...
void DropshipLoadoutClass::Run()
{
	...
	else if (ScenarioExtData::Instance())
		voiceEva = ScenarioExtData::Instance()->DropshipLoadout_StartEVA.Get(-1);
	...
	if (pHouseTypeExt->DropshipLoadout_Theme.isset())
		...
		themeIdx = ScenarioExtData::Instance()->DropshipLoadout_Theme;
	...
	while (!pressedSpaceKey)
	{
		...
		if (command == 0 && commandManager)
			command = (int)commandManager->Input();
		...
		if (WWMouseClass::Instance())
		{
			RectangleStruct mouseRect = WWMouseClass::Instance->TacticalBuffRect;

			for (auto button : buttonsList)
			{
				if (button && mouseRect.X >= button->Rect.X
					&& mouseRect.X <= (button->Rect.X + button->Rect.Width)
					&& mouseRect.Y >= button->Rect.Y
					&& mouseRect.Y <= (button->Rect.Y + button->Rect.Height))
				{
					buttonID = button->ID;
					break;
				}
			...
			if (bDragPending)
			{
				...
				if (dist >= 15)
				{
					...
					if (startingDragDropSoundIdx >= 0)
						VocClass::PlayGlobal(startingDragDropSoundIdx, Panning::Center, 1.0);
								...
								if (pTargetUnit == nullptr)
								{
									if (bDraggedIsFixed)
									{
										...
										if (endingDragDropSoundIdx >= 0)
											VocClass::PlayGlobal(endingDragDropSoundIdx, Panning::Center, 1.0);
									}
									else
									{
...
void DropshipLoadoutClass::HandleInput(int command, int buttonID)
{
	...
	if (command == VK_ESCAPE)
	{
		...
		for (size_t i = 0; i < dropshipBayChosenUnitsLists.size(); ++i)
		{
			...
			dropshipBayChosenUnitsLists[i] = std::move(newUnits);
			dropshipBayFixedUnitsLists[i] = std::move(newFixed);
		}
...
void DropshipLoadoutClass::Render(DSurface* pSurface)
{
	...
	for (int i = 0; i < nSidebarCameos; i++)
	{
		...
		if (showHighlight)
		{
			...
			pSurface->Fill_Rect_Trans(&newRectangle, &foreColor, 255);
		}
	...
	pSurface->DSurfaceDrawText(buffer, &windowRectangle, &creditsLabel, foreColor, 0, style);
	...
	pSurface->DSurfaceDrawText(buffer, &windowRectangle, &pressSpaceLabel, foreColor, 0, style);

	// Draw Dragged Cameo
	if (bIsDragging && pDraggedUnitType)
	{
		auto const pTypeExt = TechnoTypeExtContainer::Instance.Find(pDraggedUnitType);
		{
			auto const pPCXSurface = pTypeExt->CameoPCX.GetSurface();
			...
			pSurface->Fill_Rect_Trans(&newRectangle, &dragBorderColor, 255);
...
void DropshipLoadoutClass::DrawTooltip(DSurface* pSurface)
{
	...
	BitFont::Instance->Color = static_cast<WORD>(Drawing::RGB2DWORD(255, 239, 99));
	...
	if (maxLimit > 0 && !isHoveredFixed)
	{
		...
		BitFont::Instance->Color = static_cast<WORD>(Drawing::RGB2DWORD(255, 255, 255));
		BitText::Instance->DrawText(
			BitFont::Instance,
			pSurface,
			availLabel.c_str(),
		...
		COLORREF availColor = Drawing::RGB2DWORD(255, 255, 255); // White

		if (availableCount == 0)
			availColor = Drawing::RGB2DWORD(255, 0, 0); // Red
		else if (availableCount * 2 <= maxLimit)
			availColor = Drawing::RGB2DWORD(255, 255, 0); // Yellow
	...
	if (!isHoveredFixed)
	{
		...
		// Draw value (Red / Yellow / White)
		COLORREF costColor = Drawing::RGB2DWORD(255, 255, 255); // White

		if (!isHoveredInDropship)
		{
			if (currentMoney < cost)
				costColor = Drawing::RGB2DWORD(255, 0, 0); // Red
			else if (currentMoney < cost * 2)
				costColor = Drawing::RGB2DWORD(255, 255, 0); // Yellow
		}
	...
	if (!descStr.empty())
	{
		...
		BitFont::Instance->Color = static_cast<WORD>(Drawing::RGB2DWORD(200, 200, 200)); // Light Gray
		BitText::Instance->DrawText(
			BitFont::Instance,
			pSurface,
			descStr.c_str(),
...
int DropshipLoadoutClass::GetCarrierSizeLimit(int carrierIdx)
{
	...
	if (pSWTypeExt)
	{
		...
		if (auto pHouseExt = HouseExtContainer::Instance.Find(HouseClass::CurrentPlayer))
		{
	...
	{
		for (int limit : ScenarioExtData::Instance()->DropshipLoadout_Carriers_SizeLimit)
		{
			sizeLimits.push_back(limit);
		}
	}
	...
	if (configuredLimit == 0)
	{
		...
		{
			for (auto carrier : ScenarioExtData::Instance()->DropshipLoadout_Carriers)
			{
				carriers.push_back(carrier);
			}
		}
...
void DropshipLoadoutClass::SaveCargo()
{
	...
	if (pSWTypeExt)
	{
		...
		pHouseExt->DropshipLoadout_SWInitialUnits = std::move(newInitialUnits);
	...
	else
	{
		...
		if (this->bAddUnusedMoneyToPlayer.isset())
			...
			addUnusedMoneyToPlayer = ScenarioExtData::Instance()->DropshipLoadout_AddUnusedMoneyToPlayer;
		...
		else
		{
			...
			else
			{
				...
				if (pHouseTypeExt->DropshipLoadout_Money.isset())
					...
					dropshipLoadout_InitialMoney = ScenarioExtData::Instance()->DropshipLoadout_Money;
