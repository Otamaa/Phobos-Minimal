...
void CreateUnitTypeClass::LoadFromINI(CCINIClass* pINI, const char* pSection)
{
	...
	if (exINI.ReadString(pSection, "CreateUnit.Mission"))
	{
		...
		{
			this->Scatter = false;
			this->UnitMission = result;
		}
	}

	if (exINI.ReadString(pSection, "CreateUnit.Mission.AI"))
	{
		auto result = MissionClass::GetMissionById(exINI.value());
		if (result == Mission::None && IS_SAME_STR_(exINI.c_str(), "scatter"))
		{
			this->AI_Scatter = true;
		}
		else if (result != Mission::None)
		{
			this->AI_Scatter = false;
			this->UnitMission = result;
		}
	}
}
...
bool CreateUnitTypeClass::Serialize(T& stm)
{
	return stm
		...
		.Process(this->Scatter)
		.Process(this->AI_Scatter)
		.Success();
}
